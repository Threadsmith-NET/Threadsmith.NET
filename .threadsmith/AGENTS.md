...
- `code_explore` uses concise source-first Markdown by default; output-format changes are host-owned per-session diagnostics rather than repository configuration. `tools:codeExplore:inspectCodeExploreOutput` controls only whether future TUI tool-completion blocks include code_explore output. Arbitrary scalar tool settings live under `tools:config:<tool-id>`. `csharp_script` documents `timeout_ms`, `max_output_bytes`, and comma-delimited `allowed_assemblies`; it remains default-disabled and requires trusted execution regardless of repository data.
