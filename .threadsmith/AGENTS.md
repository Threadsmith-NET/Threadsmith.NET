...
- `tools:codeExplore:outputFormat` selects structured JSON or bounded Markdown model-visible `code_explore` results, while `tools:codeExplore:inspectCodeExploreOutput` controls only whether future TUI tool-completion blocks include code_explore output; interactive overrides are host-owned per-session state. Arbitrary scalar tool settings live under `tools:config:<tool-id>`. `csharp_script` documents `timeout_ms`, `max_output_bytes`, and comma-delimited `allowed_assemblies`; it remains default-disabled and requires trusted execution regardless of repository data.
