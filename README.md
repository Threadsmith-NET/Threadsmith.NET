...
The built-in repository inspection and semantic tools do not directly edit, create, or delete repository code. An approved `run_process` command is the exception: the child process runs in the repository working directory and may have direct filesystem side effects that do not pass through Threadsmith's staged mutation, diff-approval, or rollback lifecycle. For host-managed changes, the model proposes the complete structured plan up front and the user or plan policy approves that whole plan once. The same run then selects the earliest incomplete step and requests only its next coherent mutation batch through proposal-only `propose_mutations`. Each batch is validated against active-step scope, staged in a private workspace, shown as an exact diff, separately authorized under mutation policy, transactionally applied, and validated before the host continues the step or advances to the next one. `csharp_script` executes code but is not a file-editing mechanism; `propose_plan` requests this governed workflow but does not perform it.
