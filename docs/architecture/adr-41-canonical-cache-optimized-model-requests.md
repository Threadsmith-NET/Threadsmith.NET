...
- The host owns closed structured system, developer, user, assistant, tool, and request-local host-context messages. Stable host policy, confined repository instructions, and phase policy precede chronological complete turns, bounded repository memories, governed state/evidence, and the current user input. Layout version 2 explicitly distinguishes request-local host context from prefix instructions so changing memories/evidence cannot invalidate preceding history. Adapters preserve that chronology while using protocol-supported roles; Anthropic keeps actual system/developer instructions in its native system prefix.
