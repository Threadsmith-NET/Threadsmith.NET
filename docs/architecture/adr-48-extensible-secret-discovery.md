...
Both JSON providers use strict UTF-8 and bounded size, depth, and property counts; empty/non-string values and case-insensitive duplicate properties fail closed. User-store access is governed by the operating system. Threadsmith neither inspects nor changes ownership, Windows ACLs, or Unix permission bits, and does not reject an otherwise readable user store because another principal has access. `UserOwned` identifies the configured user source rather than an operating-system ownership assertion. Environment lookup derives and reads one exact variable and never enumerates the environment. The ordinary and repository-excluding configuration roots use a filtered `THREADSMITH_` provider that excludes the complete normalized `secrets` subtree, so compatible environment values never become generally accessible `IConfiguration` entries.
