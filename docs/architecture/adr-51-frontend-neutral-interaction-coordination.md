...
- concrete key handling, terminal serialization, cell measurement, width-aware
  ...
  the concrete TUI frontend;
- terminal-library types may be referenced only by the TUI side of this
...
ADR-15 recorded the former inline frontend and native scrollback; ADR-62 now owns the interactive frontend decision, composer ownership, and real-terminal behavior. This decision originally changed ownership and dependency direction without changing user-visible interaction behavior.
...
- Current and future frontends share one implementation of commands, approvals,
  ...
  Markdown safety.
- The concrete TUI depends on `Threadsmith.Interaction`; Interaction never directly
  references a frontend, persistence implementation, provider adapter, or terminal package.
- ADR-62 retired the original frontend's delegating compatibility facades.
