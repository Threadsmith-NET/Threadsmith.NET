...
## Configurable semantic qualification refinement

The calibrated `0.47` value is the default for `tools:config:memories:SemanticMinimum`, not a fixed embedding-generator policy. Normal machine, user, repository, session, CLI, and `THREADSMITH_` environment layering supplies the effective value. It must be a finite double in `[-1, 1]`; semantic cosine qualifies only when it is strictly greater than that value. Raising the minimum makes semantic retrieval more selective, lowering it admits weaker matches, and `1` admits no semantic candidate. The lexical branch and its qualification remain independent.
