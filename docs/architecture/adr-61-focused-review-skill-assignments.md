...
`inherit` passes the parent's enabled, permitted tools to a `SharedWorkspace` child, including process execution, file tools, skills and further delegation when available. Explicit `readOnly` keeps a narrower non-network inspection surface. Nested calls preserve caller scope and model provenance through the shared pipeline; waiting parents yield scheduler permits and reacquire them before resuming. Caller snapshots are transient. Explicit host resume and continuation use the existing durable checkpoint and current-session revalidation, preserving host-action workflows after the calling request ends.
