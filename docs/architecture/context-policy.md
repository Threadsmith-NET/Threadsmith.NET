...
Threadsmith.NET assembles each model request from explicit state and bounded conversation continuity. The model-context order is request-owned provider instructions when present, stable host policy, the applicable repository instruction/append bundle, canonical native tool definitions, phase policy, additional prefix instructions, recent complete exchanges, included repository memories/preferences, governed task/state/evidence/output requirements, current user input, and admitted continuation messages. Native tool definitions and provider instruction/schema fields remain separate request fields where the transport supports them, but their JSON property positions do not define model-context order. See [ADR-41](adr-41-canonical-cache-optimized-model-requests.md) and [request-order operations](../operations/cache-optimized-context.md#request-order). `/context map` reports final prepared request accounting in that host-owned order, while `/context inspect` retains assembly and reduction diagnostics.
