...
- initial evidence omitted before its first delivery.
...
`ChildAgentHistory` tracks inherited evidence as its first group, followed by completed call/result batches plus their progress or correction messages. The common active-turn compactor can replace an older, previously delivered prefix with working notes, using child-specific token/percentage triggers and a recent-context target. Initial role/task/instruction messages and user steering are never eligible. Inherited evidence becomes eligible after its first verbatim delivery and retains its original IDs in the archived evidence index. The newest batch remains exact. Replacement must reduce the fully estimated request by the configured minimum, including the summary and archived evidence index. A failed attempt leaves the same messages active and observes the configured interval before retry. `HistoryRewriteGeneration` invalidates provider continuation state after each replacement. Normal request prefixes remain stable between replacements.
...
The summary request uses the same optional global compaction profile and reasoning as the main loop, selected by trusted `context:activeTurnCompaction:profileId` and `reasoningLevel`. App passes one immutable candidate descriptor through the runner factory; configured summaries use the repository-excluding provider and instructions even when the task model uses ordinary routing. Without a global profile, summaries use the child's selected model and reasoning. The task model still owns the pressure trigger and subsequent ordinary requests. Shared deployed compaction prompts advertise no tools or requirements on the child's eventual response. Summary usage contributes to both session and child totals. An ActivitySource named `Threadsmith.Execution.ChildAgentCompaction` records before/after estimates and outcome without source content. History tuning lives in trusted `agents:delegation:compaction`; see the [user guide](../user-guide.md#subagent-history-and-evidence).
