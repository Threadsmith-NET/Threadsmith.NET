...
- `MutationProposalRepairAttempted`: historical compatibility event containing run id, retry attempt number, maximum attempts, and sanitized host-validation reason. Current mutation correction uses transient model-visible corrective messages and does not emit this event; the type remains readable for durable history.
