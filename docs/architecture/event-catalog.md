...
- `PlanRevisionRequested`: owning run id and sanitized revision instructions.
...
- `MutationProposalStarted`: run id, attempt number, and maximum attempts when approved-plan execution enters or retries the governed mutation-proposal model turn. It contains no diff, file content, raw model payload, or replacement text.
- `MutationProposalRepairAttempted`: run id, retry attempt number, maximum attempts, and sanitized host-validation reason when a repairable model-authored mutation proposal failure is being retried with correction evidence. It contains no diff, file content, raw model payload, or replacement text.
...
Conversation event payloads contain metadata and rationale only. Sanitized message bodies and memory content stay in the dedicated archive tables or content-addressed artifact store.
