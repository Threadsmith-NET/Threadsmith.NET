...
## Repository and solution lifecycle
...
- `SemanticRefreshStarted`: refresh/workspace identity, the initial reason and mode captured when the refresh sequence starts, initial coalesced changed-file count, and captured dirty version. Follow-up work retains the same refresh identity and may strengthen the sequence's aggregate reason or mode before completion. `ExternalChange` is distinct from `HostMutation`, `UserAdmission`, `Manual`, and `Recovery`; the event contains no paths, source, watcher state, or Roslyn/MSBuild object.
- `SemanticRefreshCompleted`: the same correlation plus the converged aggregate reason and strongest mode across the refresh sequence, converged dirty/applied versions, resulting semantic confidence, and non-negative authoritative duration. Reduced confidence is a successful result and does not become infrastructure failure. Initial `SemanticLoadCompleted` remains a distinct lifecycle fact rather than being emitted again for the refresh projection.
- `SemanticRefreshFailed`: the same correlation plus the captured dirty version, last applied version, non-negative authoritative duration, and bounded sanitized failure classification/detail. It leaves the workspace dirty; raw exceptions, paths, source, and secrets are excluded.
...
## Context assembly, model selection, and planning
...
## Mutation proposal and application
...
## Delegation and child-agent lifecycle
...
- `RunSteeringPauseRequested`: run and pause ids for the first accepted idempotent request. Repeated Enter presses reuse the pending id and emit no duplicate event.
- `RunSteeringPaused`: the same correlation after the parent and every active child have reached a safe response/tool boundary or become terminal.
- `RunSteeringSubmitted`: run/pause ids, optional sequence, and whether text was submitted. The steering body is excluded and remains in the sanitized conversation archive.
