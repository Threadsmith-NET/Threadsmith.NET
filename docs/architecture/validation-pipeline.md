...
When incremental plans are enabled, successful validation of every current-plan step writes `PlanContinuationPending` instead of a terminal execution outcome. The same session/run and execution engine preserve cumulative original-file, lifecycle, budget, affected-scope, and net-diff evidence while the existing conversation loop re-enters `EvidenceCollection`. Validation artifacts retain detailed per-pass evidence; resumable active state keeps only the latest validation and compact cumulative counters instead of embedding every historical result or batch diff. The model receives the latest cumulative execution receipt as current-run host context, independently of conversation-history mode, plus current repository tools. It either calls `propose_plan` for the next tranche or confirms completion with `complete_objective`. The latter reruns the configured stages once against the union of affected paths, projects, inventories, and required stages accumulated across tranches. Failure records an accurate terminal failure rather than success. Ordinary text, including questions and blockers, leaves the boundary resumable without a success outcome. The plan schema and output formatter are unchanged. There is no plan-count cap; existing execution budgets, cancellation, and policy govern further tranches.

With incremental planning enabled, an implementation or validation-correction turn may call `request_replan` with one reason instead of proposing mutations. `PlanReplanningPending` retains applied work, current validation and failure evidence, and cumulative validation scope while the main evidence/planning conversation investigates. It cannot complete the objective. A replacement plan uses existing sanity/approval and exact-diff authority. Any existing diagnostic baseline is retained across replacement and subsequent tranches, preventing introduced failures from being reclassified as pre-existing; only the transactional baseline advances to current bytes. Replanning creates no new validation runner or corrective loop.
