...
Malformed `propose_plan` tool arguments and repairable sanity issues, such as a missing edit target or ambiguous bare file name, receive bounded transient model-visible corrective messages controlled by `execution:maxCorrectiveTurns`. They share one logical planning-turn budget and do not modify durable task constraints. Non-repairable path escapes, protected paths, policy denials, and exhausted correction budgets fail closed. Only after sanity passes does `/plan-policy` / `planning:approvalPolicy` decide whether to prompt or emit policy auto-approval. This approval boundary remains separate from exact-diff mutation approval.
...
6. Publish bounded `PreMutationAnalysisCompleted` summary counts and return blocking diagnostics to the model as repair evidence. Malformed `propose_mutations` schema payloads and typed repairable proposal validation failures continue through transient model-visible correction under `execution:maxCorrectiveTurns`; correction text never becomes a durable task constraint. Passing this gate only permits private staging and approval review; it is not acceptance evidence.
