...
6. With configured models, confirm normal approved implementation and correction turns select implementer preparation through the mutation proposal application and delegation coordinator. This actual mutation protocol remains separate from ordinary replies: the child produces only a proposal and follows existing proposal validation and corrections. With no configured models, the offline flow keeps the direct proposal path.
7. Hold the child before its authoritative join and confirm no parent staging occurs. Cancel or fail the join in separate runs and confirm the proposal is not staged.
8. Let preparation join successfully. Confirm the parent stages the proposal, obtains exact-diff authorization, applies it transactionally, and runs validation and corrections through the existing execution lifecycle. No automatic parallel worktree application is implied by this preparation.
