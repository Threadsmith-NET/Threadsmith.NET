...
11. If work remains, the model proposes one next tranche through the same plan formatter and approval path. Otherwise an explicit `complete_objective` call with exactly `{}` requests completion. The host reruns configured validation over cumulative affected scope and terminalizes successfully only when that pass succeeds; malformed arguments use corrective feedback and validation failure records failure. Ordinary text and questions must not claim success. Additional plans are not limited by count; existing execution budgets and policy remain in force.
...
1. In Conversation-aware mode, complete several turns establishing a requirement, correction, decision, and unresolved question without calling a memory operation. Confirm sanitized visible user/assistant messages are archived and zero memory entries are created.
2. Ask a follow-up within the bounded recent window. Confirm complete turns remain chronological and the exact current user request is preserved. No automatic requirement/finding/completion snapshot is inserted.
3. During a long tool-driven user request, cross active-turn context pressure. Confirm the model-generated summary covers only eligible already-delivered tool groups, preserves current instructions and recent exact evidence, and creates no durable memory.
4. Cancel or fail a summary attempt. Confirm existing continuation remains active, request-capacity checks still apply, and no partial summary is adopted.
5. Explicitly save a concise relevant repository note, then switch to Governed-memory-only. Confirm relevant explicit memory can appear with no raw prior messages. Switch to Stateless and confirm all prior memory/history is omitted.
6. Inspect final selection, token/sensitivity exclusions, branch/query/cache diagnostics, and actual-submission receipts separately. Previewing context cannot increment inclusion statistics.
7. Restart, resume, and clone sessions containing both visible transcripts and historical automatic snapshots. Confirm archive order/mode/valid governed state restore while retired automatic items never reenter the prompt. Explicit repository notes remain repository-local and independent of the old transcript.

**Verifies:** bounded archive continuity, model-generated active-turn compaction, explicit repository recall, three-mode assembly, token/sensitivity policy, truthful inspection/accounting, and restart/clone exclusion of retired automatic memory. Hidden reasoning, secrets, provider wire types, and raw tool payloads never become automatically promoted conversation facts.
...
1. In a repository, complete several conversation turns that create sanitized archive messages, explicit repository memories, evidence, usage, a context inspection, and a non-default valid model/reasoning selection. Record the active session ID, restart Threadsmith, run `/resume <session-id>`, and confirm the same conversation mode, explicit repository memories, valid evidence, usage, model, reasoning, context limit, and paused execution status are reconstructed before the composer opens. Confirm no raw provider transcript or hidden reasoning is replayed.
...
5. Run `/new`. Confirm the prior session is durably resumable, a new ID becomes active, repository trust/configuration/solution/tool/policy remains effective, and conversation, evidence, run/checkpoint, usage, context inspection, transient activity, and continuation/cache state are empty. Explicit repository memories remain available under relevance/mode policy; no prior-session transcript or retired automatic snapshot appears in the canonical model input.
6. Build a source session with multiple turns, explicit repository notes, valid evidence, usage, and model/reasoning, then run `/clone`. Confirm the source is checkpointed, a new top-level session is atomically activated, copied governed context has new session-local identities plus source provenance, and output includes a directly copyable `/resume <source-session-id>` line.
7. Add different turns to the clone, resume the source through the printed command, and add different source turns. Resume each again and confirm independent post-clone histories, usage, evidence, and context generations with no automatic merge; explicit repository notes remain shared within the repository and retired automatic snapshots remain excluded.
...
10. Set `tui:showOperationDurations=false`, exercise user/repository precedence and restart, and repeat Markdown/tool/MCP continuations. Confirm duration suffixes and periodic refresh stop while `THINKING`/`TOOLS`/`MCP`, source, outcome, and sanitized detail remain unchanged. After final Markdown output, confirm fixed footer and agent-header status remain correct under resizing, styling suppression, and redirected output.
11. Resize before rendering across wide and narrow windows. Confirm Unicode-cell-aware wrapping, hanging list indentation, exact code whitespace, stable quote/rule markers, and deterministic table-to-labeled-row degradation; retained text remains selectable.
12. Inject parser, semantic-validation, layout, and adapter failures with controls in the source; cancel/fail during collection, before block closure, during elapsed activity refresh, and during shutdown. Confirm accepted exact text remains upstream, terminal-safe source is displayed exactly once where fallback is needed, no raw control reaches `IInteractionSurface`, activity/details/final markers are not lost, and run/composer state is honest.
...
15. Run package, architecture, session-status, and existing operation-duration and semantic-answer focused regression suites. Preserve their timing/detail/configuration/status assertions. Confirm the centrally pinned Markdig version/license/dependency footprint is approved for all release RIDs, Markdig remains in Interaction, the host semantic document contains no parser/backend/activity/status types, and TUIKit consumes `IInteractionSurface` without changing collection, timing, activity-detail, or status contracts.
16. Repeat default rendered mode and explicit source mode in maintained Windows Terminal plus Linux/macOS/SSH/multiplexer coverage using system/light/dark/custom themes, elapsed timing enabled/disabled, reviewed activity details, session status, retained selection, `Ctrl+C`, 10 KB/100 KB paste, resize, long responses, tool continuations, links, tables, code, and cancellation. Confirm no activity/status/detail regression, terminal cleanup failure, duplicate answer, or syntax-highlight requirement.

**Verifies:** retained interactive projection, semantic themes and status contracts, monotonic request/tool/MCP durations, bounded refresh, source/outcome/detail markers, configuration and transient activity lifecycle, direct Markdig adapter, closed host-owned semantic document, default complete-block presentation, terminal-safe streamed source/fallback, opaque non-model projection compatibility, flush-before-visible-event-boundary ordering with periodic in-place activity refreshes explicitly non-boundary, raw transcript authority, safe content policy, and deterministic fallback.
...
3. Generate the human-readable third-party notices and SPDX/CycloneDX SBOM twice from identical approved inputs. Confirm byte-stable ordering/digests, full closure coverage, component/source/version parity, inclusion of MIT, BSD-2-Clause, Apache-2.0, and applicable SQLitePCLRaw NOTICE material, and absence of retired frontend packages. Confirm an omitted, duplicate, malformed, oversized, or unapproved legal input fails safely without network lookup or script execution.
...
1. Open a repository and explicitly save a concise note through the single `memories` tool and through manual remember. Verify the actual advertised schema contains only required closed-enum `action` and optional nullable `id`/`text`; invalid combinations/unknown fields fail without writes. Ordinary conversation, approval, mutation, rollback, failed run, new session, and restart create zero notes by themselves.
2. List/inspect through both terminal frontends and headless interfaces. Confirm stable IDs, current text, model/manual origin, creation/update time, inclusion count, last inclusion, and embedding identity/availability without vectors. Repeated list/inspect does not embed or record use.
3. Add a normalized duplicate, perform an unchanged update, target an unknown ID, and update to another entry's exact text. Confirm no eviction/recency/counter changes for no-ops; unknown/conflicting writes do not merge or delete notes. Meaningful update preserves ID, atomically replaces text/vector, advances revision, and resets usage.
4. Exercise the 2,000-character and complete 256-token boundaries. Oversize, truncated, failed, cancelled, or invalid embeddings leave previous content/FTS/usage/capacity unchanged; no accepted text is only partially embedded.
5. Retrieve exact identifiers, paraphrases, short follow-ups with active task intent, and unrelated requests. Verify qualified FTS5/BM25 and compatible semantic ranks fuse, returning zero to three by default with stable ties and no usage/age/origin boosts. Stateless and disabled/denied-tool policy omit injection; sensitive and over-budget entries cannot bypass normal provider/context constraints.
6. Preview, submit, retry, resume, and continue tool rounds. Confirm only actual provider submission records usage, once per run/content revision; delayed receipts cannot increment updated/deleted content. Inspect branch selection versus final inclusion versus receipt outcome. Accounting failure does not cause provider retry.
7. Fill configured capacity and lower it. Confirm transactional deterministic eviction, seven-day recency protection when older candidates exist, decaying historical frequency, oldest-protected fallback, capacity-one safety, and duplicate retries without eviction. Manual and model notes receive the same policy.
8. Update/remove a note between tool rounds. Confirm the rebuilt next request omits obsolete memory blocks and rejects continuation retaining deleted current context. Existing historical messages remain unchanged.
9. Migrate a disposable legacy database. Verify an integrity-checked SQLite backup, import only active explicit manual provenance, drop automatic/inactive entries, preserve oversized manual text for inspection/lexical recall, reset usage, enforce capacity, and rebuild compatible vectors outside migration/write locks. Restart/resume/clone cannot resurrect dropped automatic facts.
...
**Verifies:** explicit operation authority, repository-local bounded SQLite/FTS/vector consistency, complete-input embedding validation, calibrated hybrid retrieval, final-dispatch accounting, deterministic eviction, migration/recovery, continuation correctness, and absence of automatic promotion.
...
3. PR mode uses `pr_fetch` through normal policy with `kind:"diff"` for the complete lead review handoff. A URL-only call selects the enabled account using configured URL patterns, with adapter defaults covering all organizations and repositories. Overlapping matches request an explicit account; missing matches fail before acquisition. Separately ask for the PR's changed files with `kind:"inventory"` and confirm it returns every added, modified, removed and renamed file without requesting the diff. Confirm the review file list excludes unrelated base-only Terraform changes. For a review, acquire inventory and diff together in one `kind:"diff"` call before delegation, supply the acquired evidence to specialists, and confirm `pr_fetch` is absent from every child tool catalog. Keep omissions/failed coverage explicit. Failure must not substitute branch comparison or change the checkout.
4. Confirm inventory and diff calls use separate snapshots and return complete results without cursors. Repeated identical calls in one model execution receive corrective feedback, authorized cache reads perform no HTTP, explicit refresh replaces the snapshot, narrowed policy still denies access and owner cancellation cleans up. Repeat for GitHub and Bitbucket Cloud with fixture credentials and a manually authorized live account.
...
5. Use `/skills list`, `refresh`, `inspect`, `verify`, `enable`, `disable`, and existing workflow subcommands. Confirm their command contracts remain available.
...
## Scenario AX — Ordered Context Usage Inspection

1. Prepare a primary-agent request with multiple appended prompts, applicable parent/child `AGENTS.md` files, eligible native/MCP tools, included memories, and conversation/evidence. Run `/context map`. Confirm a large retained modal uses native TUIKit charts to show actual prepared order within each request container, names individual instruction/append sources, and identifies separately transported fields without inventing a provider-internal ordering.
2. Scroll the flat stack to identify individual instruction/append files, tool definitions, messages, and tool results by safe labels and sizes in prepared order. Verify empty entries and parent totals are omitted, with no nesting or expansion controls. The fixed sidebar retains larger category groups, aggregates every occurrence, ranks the largest categories first, and shows total input, model window, percentage used, and category percentages of input. Scrolling does not change totals.
3. Compare the map with the primary context indicator for the same request. Continue through tool rounds, correction, and compaction, then reopen the map. Confirm newly included content is counted, replaced history is removed, and the request identity/model/estimate agree. Child and auxiliary compaction-generation calls do not replace the primary request being explained; cumulative usage continues to account for them.
4. Verify estimation basis and provider framing/replay contributions are identified without exposing private content. Excluded tools, memories, evidence, and archive-only conversation do not consume displayed input. A fresh or resumed session without detailed request metadata reports unavailable detail rather than zero or reconstructed context.
5. Keep the modal open while normal execution progresses. Confirm its captured rows remain stable, ordinary activity completes, and opening/navigation/closing performs no model call, tool execution, context assembly, instruction reload, memory retrieval, or configuration change. Resize and close it; preserve the draft, focus, and established interruption behavior.

6. Double-click the Context usage meter at the top-right of MAIN's output window. Confirm it invokes the same inspector as `/context map`, exactly once, including during active execution and with an unsent draft. Repeat on the label, bar, and numeric summary after resizing. Single clicks, other buttons, dragging, clicks outside the meter, a third click, or disabled application mouse capture do not open another inspector. Existing modal/startup ownership is respected; a child meter does not open unrelated MAIN accounting.

**Verifies:** faithful request attribution, actionable source sizes, complete additive accounting, primary-request correlation, native graphical presentation, shared command/gesture invocation, bounded read-only inspection, and frontend/execution compatibility.

---

## Scenario AY - Governed Jira Issue Reads

1. Configure trusted Jira Cloud accounts for a scoped API token and an unscoped API token using logical user-owned secret references. Confirm repository configuration can disable accounts or lower limits but cannot add, enable or rebind an account, endpoint mode, tenant, cloud ID, browse alias, email or credential reference.
2. Enable `jira` through ordinary outbound consent and read an authorized issue with `kind:"read"` using a key, a decorated standard `/browse/{key}` URL and an exact trusted custom-domain browse alias. Confirm the scoped account uses only `api.atlassian.com/ex/jira/{cloudId}`, the unscoped account uses only its configured `.atlassian.net` site, and aliases are never contacted.
3. Confirm one Get issue request selects only summary, description and updated fields without history updates, redirects, discovery, comments, attachments, media or linked-resource fetches. Verify requested/current key, ID, canonical site URL, timestamps, readable body and untrusted provenance.
4. Exercise null, empty, rich, unknown, malformed and large ADF descriptions. Confirm absent versus present state, deterministic readable text, explicit unsupported-content limitations, Unicode-safe partial bodies under body/runtime/sanitization bounds, and hard failure for malformed or transport-truncated JSON. No partial result is represented as complete and no continuation is invented.
5. Exercise invalid/numeric/future kinds, malformed keys, unsupported routes, lookalike hosts, provider mismatch, ambiguity, policy denial, authentication/authorization/not-found/throttling/provider failure, timeout and cancellation. Confirm applicable rejection precedes Secrets/HTTP, no endpoint fallback/probing occurs, and errors contain no credential, authorization header or raw response body.
6. Inspect main and delegated output. Confirm running and final activity identify `read`, issue key, selected account and outcome through the shared tool lifecycle without ticket summary/description content. Ask to show the ticket description and confirm the ordinary assistant response can use the returned body and disclose limitations without a second kind or fetch.
7. Rotate a token behind the same logical reference and perform a separately permitted read. Confirm request-local resolution uses the replacement according to provider refresh semantics. Run deterministic regression, prompt/package and architecture checks, then execute the live matrix in MTP-272 without storing real credentials or private issue text.

**Verifies:** trusted Jira account routing, scoped and unscoped API-token support, URL-first issue identification, bounded ADF evidence, common tool policy/Secrets/network/output/activity integration, requested description display, and absence of ticket mutation authority.
