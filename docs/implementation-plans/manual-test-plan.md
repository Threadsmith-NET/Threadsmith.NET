...
**PR retrieval:** Configure one GitHub.com and one Bitbucket Cloud account using user-owned Secrets (reuse MTP-227 trust checks). Ask naturally for the complete changed-file list and confirm `pr_fetch` uses required `kind:"inventory"`, follows every inventory cursor, reports added/modified/removed/renamed files, performs the final metadata consistency check and makes no diff request. Review the same PR URL naturally and with `/skills use ... {"mode":"pullRequest","url":"..."}`; confirm the lead review uses `kind:"inventory"` through completion before delegation, then specialists use `kind:"diff"` only when assigned patch evidence is needed. Test adapter default URL patterns, then custom `urlPatterns` for two accounts on the same host; confirm unique selection, ambiguity feedback for overlapping matches, no-match feedback and an explicit `provider` override. Repository config must not replace trusted patterns. Compare acquired files and hunks with the provider PR view, including a base-only Terraform change, rename, deletion and binary. Verify all five existing roles/report sections. Confirm inventory and diff calls have separate snapshot/cache identities and reject a cursor reused with the other kind. Repeat an identical call in one model execution and confirm the shared corrective duplicate response, then repeat a first-page read from a permitted child and confirm cache reuse with no additional HTTP/secret resolution. Refresh explicitly and reject old cursors. Tighten tool/network permissions after acquisition and confirm cached reads fail. Exercise missing credentials, unsupported host, moving PR, bounded output and ESC–ESC during HTTP/child execution. Confirm ordinary correlated tool activities (MTP-270), no branch/working-tree writes, honest partial coverage, no branch-comparison fallback and fresh acquisition after operation end/resume. Live credentials must not enter fixtures or logs.
...
7. Start a new session, clone/resume, change the selected model after capture, and exercise unavailable/evicted metadata. Confirm captured identity/limits remain honest and historical prompt bodies are not loaded to reconstruct the chart. Repeat `/context map` with the original frontend and verify matching text totals and existing `/context`, `mode`, `inspect`, and `compact` behavior.
...
1. Request a change whose complete structured plan has two ordered steps, with the first step covering at least three independently editable files and the second covering another file. Capture the planning request and response. Confirm one `propose_plan` call contains the complete plan before approval; no plan step is generated lazily during implementation. Review and approve the whole plan once.
...
4. After the third batch passes validation, confirm only step 1 becomes complete and the host selects step 2 without another plan proposal or plan approval. Complete step 2 with a separately authorized batch. Confirm terminal success occurs only afterward and reports every completed step, cumulative changed paths, validation evidence, approval provenance, and one net final diff relative to execution-start bytes rather than concatenated historical patches.
...
10. Repeat the successful, partial, corrected, interrupted, cancelled, and hard-limit cases through TUIKit, `--tui=original`, and headless execution. Confirm ordinary next-batch review is not labeled as a correction, partial authorization reports the existing resume command, batch success is not terminal success, pending-review versus terminal state is consistent, and cancellation prevents future model requests.
11. Restore default targets (`8` mutations, `3` distinct paths, and `24000` mutation-content characters). Run the maintained build and focused orchestration/mutation/core/architecture suites. When a live model is authorized, record profile/reasoning, time to first exact diff, total duration, request count, actual mutation/path counts, completion hints, corrections, and final content verification without recording prompts, source bodies, credentials, or provider payloads.

**Expected:** Threadsmith proposes and approves one complete plan up front, then exposes useful exact diffs incrementally by host-selected approved step. Soft targets guide but do not authorize or hard-reject coherent work. Every batch uses the existing proposal, correction, exact-diff policy, transaction, validation, checkpoint, and shared presentation paths. Partial consent and interruption preserve honest durable progress, later work requires fresh authorization, and terminal success plus final net diff represent the complete approved run.
