...
10. Configure small positive `planning:incrementalPlans:targetSteps` and `targetFiles` values. Use a larger objective that needs a second tranche, confirm the continuation request contains the original objective, cumulative authoritative receipt even with history disabled, current repository tools, completed-plan count, and configured targets, then approve and execute the second ordinary plan. Confirm plan revision and ordinal advance while cumulative original-file/diff evidence and budget use remain on the same run. At `maximumPlansPerObjective`, confirm `propose_plan` is withheld and a structural attempt receives existing unavailable-tool corrective feedback. Return an ordinary explanation of remaining work or a question: neither may record success. Resume that boundary both in-process and after restart and confirm planning restarts without repeating mutations. Fail a later tranche and confirm the final receipt still reports previously completed steps, summaries, risks, and changed files.
11. Repeat the successful, partial, corrected, interrupted, cancelled, and hard-limit cases through TUIKit, `--tui=original`, and headless execution. Confirm ordinary next-batch review is not labeled as a correction, plan completion is labeled as remaining-objective assessment, partial authorization reports the existing resume command, batch success is not terminal success, pending-review versus terminal state is consistent, and cancellation prevents future model requests.
12. Restore default mutation targets (`8` mutations, `3` distinct paths, and `24000` mutation-content characters) and plan targets (`4` steps, `8` paths, and `12` maximum tranches). Run the maintained build and focused orchestration/mutation/core/architecture suites. When a live model is authorized, record profile/reasoning, time to first exact diff, total duration, request count, plan/batch counts, actual mutation/path counts, completion hints, corrections, and final content verification without recording prompts, source bodies, credentials, or provider payloads.

**Expected:** Threadsmith proposes and approves one complete plan tranche before implementing it, then exposes useful exact diffs incrementally by host-selected approved step. A validated tranche returns to the same ordinary planning loop for either one next tranche or objective completion. Soft targets guide but do not authorize or hard-reject coherent work. Every tranche and batch uses the existing formatter, proposal, correction, approval, exact-diff policy, transaction, validation, checkpoint, and shared presentation paths. Partial consent and interruption preserve honest durable progress, later work requires fresh authorization, and terminal success plus final net diff represent the complete multi-plan objective.
