...
1. Open a disposable `TrustedRead` C# repository, select its solution, and use a controlled provider that invokes `delegate_agents` with two requests containing `task`, `context`, and `toolAccess`; omit `role` for one and set it to `explorer` for the other. Use `readOnly` for one and `inherit` for the other and confirm both use Explorer behavior.
2. Hold both child providers at a barrier and confirm they overlap under the existing scheduler. Let each call an allowed inspection tool and return its own final response, without a required role template or citation GUID.
...
4. Repeat with unknown input fields, zero/excess children, empty/oversized task or context, unknown access mode, unknown or incorrectly capitalized roles, attempted provider/profile/reasoning/budget/trust/tool/approval fields, recursive `delegate_agents`, and a workflow-transition tool. Confirm each fails before unauthorized work.
...
6. Return text, JSON, whitespace, an empty reply, missing legacy role fields, and unsupported or contradictory claims. Confirm every present final body can complete without content grading or response-format repair. Inspect `AgentRunOutcome.Response` and verify claims are not promoted into verified findings or parent evidence. Fail joined-checkpoint persistence and confirm no authoritative joined response is exposed. Separately load a legacy structured checkpoint and confirm its existing validation/admission path remains supported.
7. Put sensitive evidence in parent scope while assigning a non-sensitive child. Confirm the evidence is omitted. Attempt tool access to `.git`, prohibited, outside-root, and reparse locations and confirm host policy rejects the calls regardless of final-response wording.
8. Through a controlled host-command/headless harness, cancel once while children are queued/running, once after one sibling completed, and once before the authoritative join commits. Confirm queued work never starts, every started child is observed, child identities/statuses remain inspectable, the aggregate parent result is `Cancelled`, no incomplete join becomes authoritative, and late results do not replace the cancellation boundary. Separately fail transport and confirm it is not represented as a successful empty response.
...
10. Repeat in batches within the configured child limit using `implementer`, `securityReviewer`, `testReviewer`, `performanceReviewer`, and `architectureReviewer`. Include an Explorer and two reviewers in one batch. Confirm concurrent sibling work, role system-prompt amendments, configured model selection, eligible tools, and the common `agent-response/1` marker without a body schema. Submit roles in a different order and confirm each response remains associated with its host assignment/role metadata. Repeat the tool-denial checks for every role in both access modes.
11. Have every reviewer return an empty reply, then arbitrary text or JSON with no legacy required fields. Include duplicate claims, conflicting conclusions, and assertions that checks passed. Confirm contents do not trigger repair, semantic deduplication, or promotion to verified findings. Fail one sibling's transport and confirm the others remain available in `Partial` output. Evaluate usefulness and support manually, independently of host completion status.
...
MTP-231 — Retired: original scrollback Markdown procedure

Retired: use MTP-257 for the retained TUIKit frontend.
...
MTP-233 — Retired: original scrollback activity procedure

Retired: use MTP-257 for the retained TUIKit frontend.

MTP-234 — Retired: original frontend terminal matrix
...
2. Run a search and inspect the next model request: `web_fetch` is absent before the search result and present only after eligible results exist. Without adding the result hostname to `allowedNetworkHosts`, fetch a raw result URL twice and another public HTTPS page on that exact hostname during the same run. Verify no per-URL approval appears, including after consuming the opaque result ID. Verify subdomains and unrelated hostnames still require separate authority.
...
4. For a hostname without session or saved-user approval, complete/cancel the run, start another turn or session, disable search/fetch, and switch repositories. Verify each transition removes the old search hostname authority before another network request, and a stale opaque ID cannot revive it.
5. With an explicit host direct grant on a hostname outside the current search grants, fetch the exact URL once; verify replay and an undisclosed redirect fail. For a search-authorized hostname, verify an explicit redirect chain retains its exact approved redirect scope and an unrelated search-authorized hostname alone does not permit a cross-origin redirect.
...
5. While fetch is active, have the model propose a direct URL on a hostname outside any current search grants and without an exact grant, containing a harmless query marker. Verify the prompt identifies model provenance, sanitized origin/path, query presence, and digest; select Deny and confirm zero DNS/network activity.
...
7. Select `Approve for this session` for a new hostname. Fetch another page on that exact hostname in the current and next turn without another prompt; verify subdomains still prompt, and `/new`, `/resume`, `/clone`, repository change, and tool/consent reset end session approval.
8. Select `Add to user allowed list` for another hostname. Verify the user-file `tools.allowedNetworkHosts` receives only the hostname, unrelated settings survive, and the current request and a new process/session can fetch it without a prompt. Remove the entry and verify the next ungranted fetch requires approval. Make the config unwritable and verify saving fails without fetching.
9. Run the ungranted proposal headlessly. Verify no prompt or stdin read occurs and `DirectAuthorizationRequired` is reported. Pre-authorize the exact initial URL through the headless grant surface and verify a fresh invocation succeeds.
10. Use `/fetch-authorize <initial> <redirect>` and its headless equivalent. Verify the exact redirect group still succeeds while current-message and inline single-URL routes reject an unapproved redirect.
...
4. In the trust selector, use Up/Down and Enter to choose Trusted Read.
5. In the solution selector, use Up/Down and Enter to choose `ManualTarget.sln`.
...
- The highlighted choice moves with arrow keys and Enter activates it.
...
MTP-030 - Retired: original scrollback launch procedure
...
MTP-030D - Retired: original theme selector procedure

Retired: use MTP-257 for the retained TUIKit frontend.

MTP-030F - Retired: original tool selector procedure
...
- A valid preference auto-loads without a solution prompt and shows `Remembered: <name>` in the startup splash; explicit `--solution` wins and becomes the new preference.
...
MTP-030E - Retired: composer-adjacent status procedure

Retired: use MTP-257 for the retained TUIKit frontend.

MTP-030A - Retired: repository-named composer prompt procedure
...
- The committed composer entry and streamed response remain visible once, followed by a fresh composer without redundant speaker labels.
...
1. Configure deterministic fake models with finite global, per-parent, and implementer scheduler limits lower than the submitted assignment count.
...
### MTP-167 — Parallel exploration returns unrestricted responses

1. Delegate distinct architecture, call-flow, and test-coverage questions to three read-only explorers.
2. Inspect each child's model, reasoning, context, tools, trust ceiling, budgets, immutable baseline, final response, and separate host terminal metadata.
3. Have fixtures return plain text, JSON, whitespace, empty text, missing legacy role fields, unsupported claims, and conflicting conclusions. Separately request forbidden mutation/delegation tools and simulate transport failure and cancellation.
4. Inspect `AgentRunOutcome.Response` and the durable joined result. Confirm no response-format correction request is sent and no body claims are inserted as verified parent findings.
...
1. Run `securityReviewer`, `testReviewer`, `performanceReviewer`, and `architectureReviewer` over immutable diff/evidence artifacts in batches within the configured child limit.
2. Attempt process/test execution, mutation, workflow transitions, and delegation from each reviewer in both `readOnly` and `inherit` modes. Confirm all are denied; eligible network read tools may remain only in `inherit`.
3. Return duplicate, disagreeing, unsupported, and self-resolved claims as plain text or JSON. Omit every legacy required review field and citation GUID. Repeat with whitespace and empty final replies for each role.
4. Confirm role-specific prompt amendments, selected models, and eligible tools are delivered while response bodies remain unrestricted. Fail one child's transport and confirm completed sibling replies remain available.
5. For opt-in live checks, use the harness command in `docs/operations/parallel-agents.md#live-role-evaluation`. Inspect saved responses manually for useful security analysis, test assertions, performance reasoning, or architectural advice, including omissions and unsupported claims. Compare request/tool usage and time only across comparable useful answers.

Expected: a present reviewer reply can complete even when empty or lacking structured fields. No response-format repair or automatic content-quality pass criterion applies; completion, join, configured routing, and unchanged read-only files are mechanical checks. Saved responses support manual utility evaluation, not host promotion of claims to verified findings. Reviewers remain read-only and cannot run validation processes, publish reviews, approve integration, or mark required findings resolved. Successful siblings remain available when another reviewer fails.
...
Prerequisite: use disposable user/machine configuration, a disposable repository, and controlled provider captures. Keep real credentials out of fixtures.

1. Put valid `agents:roleModels` entries in trusted user/machine configuration using the JSON shape in `docs/operations/model-providers.md`. Select enabled trusted profiles with different provider/profile/reasoning settings, restart, and assign every role. Inspect configured and effective selections, source, and fallback in assignments, checkpoints, and outcomes, plus the effective selection in `/agents <id>`.
2. For resource-limited approved assignments, exhaust a child token/tool/time budget while siblings retain capacity, then exhaust the parent aggregate budget. For ordinary `delegate_agents`, inspect recorded usage and use a real request-capacity or configured deadline boundary to stop a child; free-form replies do not disable active resource controls.
...
5. Omit a role, then omit `reasoningLevel` from a configured role. Confirm inherited/default selection for the first and the selected profile's default reasoning for the second. Vary capitalization in the provider ID and a supported reasoning name, such as `high` and `hIgH`; confirm both resolve identically. Edit a valid mapping while running: existing state remains frozen and the new mapping applies to new assignments only after restart. Confirm `/models` and `/reasoning` do not edit it.
6. Restart separately with an unknown or incorrectly capitalized role/field, a non-object entry, missing provider/profile, malformed or empty GUID, missing/disabled profile, provider/profile mismatch, unsupported reasoning, and legacy trusted `agents:roleProfiles`. Include null, empty, numeric, comma-separated, and whitespace-padded reasoning values. Confirm startup fails with a sanitized error before child activity instead of treating the mapping as absent.
7. Add or replace role mappings in repository `.threadsmith/config.json`, then add a repository provider/model entry with the same ID and a different endpoint or credential reference. Confirm trusted roles and their fallbacks still resolve and dispatch through the trusted catalog; repository-only profiles and secret stores cannot supply that route. Inspect captured destinations without writing credential values to logs.
8. In the host harness, combine an application assignment pin, role mapping, inherited preference, and compatible default. Remove each higher-precedence choice in turn and confirm the next source is recorded. Verify only `RoleConfiguration` uses the trusted catalog; pins, inherited preferences, and defaults use ordinary effective-catalog dispatch and its normal repository and eligible secret rules. Switch `toolAccess` between `readOnly` and `inherit` and confirm it cannot change that model authority. Make the preferred profile incompatible with actual context/output, tools, sensitivity, workload, or cost constraints. Confirm a compatible fallback with a reason, or failure before I/O if none exists. No request goes to an incompatible profile; ordinary replies do not require a role-specific structured-output capability.
9. Read saved checkpoints after restarting with different role configuration. Confirm their configured/effective selection, source, fallback, role, schema, and runner version remain inspectable and no interrupted delegated model stream starts automatically.

Expected: trusted role configuration is strict, requires restart, and cannot be redirected by repository content. Assignment pins precede role mappings, inherited preferences, and compatible defaults; every actual request is checked before I/O and fallback stays visible. Persisted inspection preserves selection provenance without resuming child loops. Actual authority never exceeds parent policy, empty tool allowlists deny all, and usage accounting remains correct. Resource-limited exhaustion cannot borrow from siblings. No model output changes routing, tools, trust, sensitivity, context, or budgets.
...
4. Attempt a workflow with a cycle, arbitrary expression/code, unbounded loop, nested skill, recursive agent delegation, skill-owned task/concurrency directive, overlapping workers, or a `delegate_agents` call above `maximumAgents`.
...
Before switching models, complete a request and run `/reasoning` with a different supported level. Confirm MAIN's header and F2 details show the new setting before another request, while the existing context estimate and usage counters remain unchanged. Try an unsupported level and confirm the header retains the accepted setting. Child headers retain their own request settings.
...
Expected: each admitted remote call appears in TUIKit as a live `MCP: <profile/tool> - running` block before completion, with an independent timer. One retained `MCP: <profile/tool> - <outcome>` block follows with remote invocation duration; no duplicate `TOOLS:` result is emitted. Policy/approval waiting is not mislabeled as remote duration. Retry semantics are included consistently in the logical remote boundary. Legacy data omits duration; invalid data degrades safely. No endpoint, header, token, arguments, result body, SDK type, or timer tick leaks.
...
3. Confirm SQLitePCLRaw's Apache/SQLite notice, TUIKit's MIT package license, and the supplemental embedded-font attribution/WTFPL notices appear. Confirm SBOM package identities and TUIKit's aggregate embedded-resource entry equal the exact reviewed restore closure; retired frontend packages are absent.
...
1. Confirm `--tui` and `--tui=tuikit` both select TUIKit. Confirm `--tui=original`, `--tui=pretty`, empty/unknown selectors, and duplicate selectors fail before startup. Check MCP/auth precedence without creating a terminal UI.
2. Open a trusted solution with a deliberately delayed initial semantic load. Complete required trust/solution choices before the splash. Type and paste `hello` while the splash is active; confirm it is discarded and no run/model call or later submission results. Successful phase and remembered-solution text stays only in the splash. After readiness, type `next draft` and confirm it remains editable and unsent. Test queued input separately during an external semantic refresh after startup.
...
### MTP-259 - TUIKit Command Palette and Autocomplete

Prerequisites: run `threadsmith --tui=tuikit` in a supported interactive terminal with a disposable repository. Record OS, terminal, dimensions, and theme. Repeat at 40 x 12, 80, 120, and 200 columns with default, configured, high-contrast, ASCII, and `NO_COLOR` styles. Include Linux/macOS and tmux/SSH where available.

1. Clear the ordinary draft and press F3. Search `reasoning effort`, navigate with arrows/PageUp/PageDown/Home/End, and confirm the selected usage/description. Press Enter and confirm `/reasoning` is inserted without opening a workflow or submitting. Append ` high`, then press Enter and compare with manually entering the same command.
2. Type `/rea`; accept with Tab, then repeat with Enter. Confirm one undo restores `/rea`, no space is appended, and submission history changes only after explicit submission. Press Escape on a visible list and wait through several repaint frames; confirm it stays dismissed until the destination changes. During an active run, dismissing discovery must not arm cancellation; the later run-cancellation chord still requires two Escape presses.
3. Exercise F3 cancellation using both F3 and Escape. Try exact names, arguments, leading spaces, prose, multiline paste, selection, secondary/steering prompts, and transcript focus. Confirm the draft is not replaced. Verify hidden-list Tab indentation, history, modified Enter, shifted selection, mouse caret movement, and undo/redo retain normal behavior.
4. Paste multiline Unicode into the palette query, then delete combining characters and emoji. Test 256-character and oversized queries, no matches followed by navigation and query replacement, Ctrl+A/Ctrl+C query copying, F6 detail copying, Ctrl+C with no selection, and F12 mouse handoff. Confirm query paste never enters the conversation draft and delayed clipboard results cannot reach a closed/replaced destination.
5. With suggestions visible near a wrapped/right-edge caret, resize smaller and larger. Confirm all shown names remain readable, selected rows remain distinct without color, the activity/status rows are untouched, and shrinking below 40 x 12 preserves drafts and blocks editing. Open another selector and verify it owns all input until closed.
...
Expected: discovery only edits eligible command names; shared command execution begins only after separate submission. Exact drafts, focus, bounds, Unicode, themes, reserved rows, and terminal lifecycle remain intact. Record physical observations separately from headless test results.
...
1. Ask exact, paraphrased, short-follow-up, and unrelated questions. Inspect branch contributions, selected IDs, and final context; compare current steering with active task intent. Confirm zero to three relevant situational notes by default, plus applicable standing preferences regardless of query similarity, safe Unicode/punctuation handling, stable ties, and no frequency/origin boost.
...
2. With controlled counters, verify input 100 plus cache-write 200 plus cache-read 300 becomes total input 600. Verify cumulative output 10 then 25 is accounted as 25, and omitted counters stay unknown. Supply final `output_tokens_details.thinking_tokens`, then omit it and report zero in separate requests. Confirm reasoning appears only when reported, remains included in output, and never increases total tokens or cost a second time.
...
**Steps:** List/inspect skills; confirm `review` is the only maintained code-review package. Invoke `/skills use Maintained:review {"baseBranch":"main"}` after creating committed, staged, unstaged and untracked fixture changes. Exercise all five prompted reviewer roles through `delegate_agents` and a Claude-compatible package. Repeat with `maximumAgents` below five and at five, vary concurrency independently, and repeat with operational-limit enforcement disabled.

**Expected:** All five focused roles are accounted for when the request is admissible with existing model routing. A lower positive `maximumAgents` rejects the five-assignment call, while concurrency only queues admitted work. Ordinary outputs and permissions remain unchanged. Reviewers inspect one stable target and report whether tests or processes actually ran. The report uses fixed headings and explicit exclusions or partial coverage.

## MTP-267 — Remote and requirements review provenance

**PR retrieval:** Configure one GitHub.com and one Bitbucket Cloud account using user-owned Secrets (reuse MTP-227 trust checks). Ask naturally for the complete changed-file list and confirm `pr_fetch` uses required `kind:"inventory"`, follows provider inventory pagination internally, reports added/modified/removed/renamed files, performs the final metadata consistency check and makes no diff request. Review the same PR URL naturally and with `/skills use ... {"mode":"pullRequest","url":"..."}`; confirm the lead review makes one `kind:"diff"` call before delegation and supplies that evidence to the specialists. Confirm `pr_fetch` is absent from every child tool catalog. Test adapter default URL patterns, then custom `urlPatterns` for two accounts on the same host; confirm unique selection, ambiguity feedback for overlapping matches, no-match feedback and an explicit `provider` override. Repository config must not replace trusted patterns. Compare acquired files and hunks with the provider PR view, including a base-only Terraform change, rename, deletion and binary. Verify all five existing roles/report sections. Confirm inventory and diff calls have separate snapshot/cache identities, and neither input nor output exposes continuation cursors. Repeat an identical call in one model execution and confirm the shared corrective duplicate response. Refresh explicitly and verify the snapshot identity changes. Tighten tool/network permissions after acquisition and confirm cached reads fail. Exercise missing credentials, unsupported host, moving PR, bounded output and ESC–ESC during HTTP/child execution. Confirm ordinary correlated tool activities (MTP-270), no branch/working-tree writes, honest partial coverage, no branch-comparison fallback and fresh acquisition after operation end/resume. Live credentials must not enter fixtures or logs.
...
6. Exercise `/skills list`, `/skills refresh`, `/skills verify`, `/skills enable`, `/skills disable`, and an existing workflow command. Recheck `/tools` filtered group toggles and MCP's individual authentication actions.
...
5. Run a skill requesting host input. Its live activity must yield to prompts, preserve outcome output, and join cancellation cleanup before session shutdown.

## MTP-271 — Ordered context usage inspector

**Prerequisites:** Disposable repository with root and nested `AGENTS.md`, two configured append files of noticeably different sizes, an eligible tool inventory with distinguishable schema sizes, and included/omitted memory fixtures. Use a deterministic provider fixture for exact request/accounting assertions and a configured provider for a small physical-terminal smoke check. Use only disposable files/tools for adjustment experiments.

1. Before the first model request, run `/context map` and verify an unavailable message. Submit a request scoped to the nested instructions with several conversation/tool exchanges, then open the map. Compare its source/container ordering and inclusion with the fixture's prepared request. Confirm both append paths and applicable instruction paths appear, and a deduplicated source is counted once.
2. At a large terminal size, verify a two-pane modal with native horizontal charts, capacity progress, readable token values, and a fixed category summary. Scroll the flat stack of individual sources in prepared order; confirm no nested groups, duplicate parent totals, or empty entries appear. Identify expensive tools, memories, instruction/append files and conversation entries directly. Verify the right-hand summary retains larger category groups and their totals against the fixture, including wrappers, separate tools/output schema, and provider overhead. Confirm category percentages use input as the denominator and overall utilization uses the captured model window.
3. Generate another tool round, a correction, and sufficient history for compaction. Reopen after each prepared primary request. Confirm request identity/total/window match the primary context indicator, the active summary replaces removed history, and child/auxiliary requests do not displace the inspected primary context. Repeat with a fixture failure/cancellation before provider submission and verify that preparation is not labeled as a successful submission.
4. In the disposable fixture, shorten the large instruction file or disable a tool through its existing workflow. Reopen without a new model request and confirm the captured snapshot is unchanged. Submit another request and verify the expected source/tool contribution changes; the inspector itself must not apply an edit, toggle, retrieval, or context rebuild.
5. With an ongoing request, open and navigate the modal while tool events and completion occur. Verify stable captured rows, continued execution, and a newer snapshot on reopen. Exercise arrows, PageUp/PageDown, Home/End, Tab category/order switching, Esc, existing Ctrl+C interruption, and F12/mouse wheel where supported. Confirm Enter/Left/Right do not expand or collapse rows. Check preserved composer draft/focus and shutdown cleanup.
6. Resize through 160x50, 120x35, 80x24, 40x12, and below minimum. Test current themes, long/CJK/combining-character paths, terminal-control labels, and a large metadata fixture. Confirm readable adaptive layout, safe labels, stable bar scale during scrolling, unchanged sidebar totals, and no unbounded off-screen surface. Recheck an ordinary existing modal's size/behavior.
7. Start a new session, clone/resume, change the selected model after capture, and exercise unavailable/evicted metadata. Confirm captured identity/limits remain honest and historical prompt bodies are not loaded to reconstruct the chart. Verify existing `/context`, `mode`, `inspect`, and `compact` behavior.

8. With application mouse capture enabled, double-click the top-right Context meter on MAIN and compare with `/context map`. Repeat on the label, bar, percentage/capacity, and compact form after resizing. Verify one shared invocation/modal, including an unavailable snapshot, active execution, and a nonempty unsent draft. Close and confirm the draft is unchanged and no gesture became model input or steering. Try a single click, right-button double-click, drag, outside click, and triple-click; these must not open an extra dialog. Disable capture with F12 and verify terminal-owned selection remains available. Confirm existing modals/startup block the gesture and child meters never open unrelated MAIN accounting.

**Expected:** The user can identify which included files, tools, and conversation components explain the primary context usage. The slash command and Context-meter double-click share one inspector path. All leaf/category/input totals reconcile in the request's stated estimation basis. Inspection is read-only, metadata-only, and responsive while normal work continues; private replay has size-only attribution and ordinary context admission/cumulative accounting remain unchanged.

**Limitations:** Token sizes remain estimates. A provider's hidden ordering between separate request fields is not observable, and private replay content must not be displayed. Record terminal dimensions, OS, theme, input protocol, provider, and physical resize/mouse/cancellation evidence separately from deterministic headless results; missing provider/terminal environments remain unassessed.

### MTP-272 - Jira scoped/unscoped reads and visible output

**Prerequisites:** Disposable authorized Jira Cloud test issue with non-sensitive rich description content; trusted user configuration for one scoped API token with classic `read:jira-work` and cloud ID, one API token without scopes, standard Secrets references, network allowlists, and permission for small live requests. When available, configure an organization custom-domain browse alias. Never place token values or private ticket text in configuration, commands, fixtures, logs or this record.

1. Enable **Jira** through `/tools` and accept its outbound consent. Read the test issue by key through the scoped account, then in a separate user operation paste Jira's actual copied standard browse link. Confirm the request uses the gateway/cloud ID, returns current identity and description, and the account has only required browse/issue-security access.
2. Repeat through the unscoped account by key and actual copied browse link. Confirm the request uses the configured tenant directly and never tries the gateway. If a real custom-domain alias is available, paste its actual copied `/browse/{key}` link and confirm Threadsmith still contacts only the configured API authority and returns the canonical `siteUrl` browse link. Record unavailable alias infrastructure as unassessed.
3. Observe main and permitted child-agent output during success, denial, timeout and cancellation. Confirm each block retains `read`, requested/current key as applicable, account and outcome/duration, while summary, description, raw ADF, email and secret reference remain absent. Ask "Show me the ticket description" and confirm the normal answer contains the readable body plus any limitations without another operation kind.
4. Use a test description containing headings, nested lists, code, table, panel, status/date, link, card/media and unsupported content. Compare readable text and omission markers with Jira. Lower body and runtime output limits in disposable trusted configuration, restart, and confirm a usable Unicode-safe prefix with `bodyComplete:false`, limitation codes and visible truncation; restore limits. Exceed the response bound with a fixture rather than private live data and confirm explicit failure.
5. Replace the test token behind the same logical reference according to the configured Secrets provider's refresh semantics, then make a new permitted read. Confirm the replacement works and the old token is not reused. Revoke a disposable credential and verify sanitized authentication failure without token inference, endpoint fallback or raw Jira response details; restore a valid credential.
6. Exercise invalid future/numeric kinds, unsupported board/search/API URLs, lookalike hosts, account ambiguity and provider/site mismatch. Confirm local actionable rejection and no credential/API request. Inspect events, diagnostics, raw-model logging policy and support-bundle output with canaries; credentials and authorization values must remain absent from ordinary evidence.

**Expected:** Both declared token modes read the same authorized Jira issue through fixed trusted authorities and the common governed tool lifecycle. URL parsing is local, progress is useful but content-free, requested assistant display works, rich/partial evidence is honest, and no search or mutation capability is exposed.
...
6. After a passing fully applied batch, return an empty mutation list with `stepComplete: true` for the same active step and confirm completion can proceed. Attempt the same response before any qualifying same-step validation, and after only another step has passing evidence; confirm bounded corrective feedback and no completion. Attempt a nonempty `stepComplete: true` batch with failed validation and confirm the step does not advance.
7. Partially authorize a multi-file or multi-mutation candidate. Confirm only authorized changes apply, actual work is validated, the run records `ContinuationPending`, the stale candidate is no longer reviewable, and no rejected work is regenerated automatically. Invoke `/validation retry`; confirm a fresh candidate uses promoted current bytes and requires a fresh exact-diff decision.
...
**Prerequisites:** Disposable repositories with configurable user and repository settings; one ignored and one unignored in-repository directory; an existing external directory; retained TUIKit and headless surfaces. Use only disposable contents because every scratchpad child is intentionally deleted.

1. Start with `scratchpad.path` absent and explicitly `null`; confirm no directory work, model guidance, or special file-tool authority. Configure a missing repository-relative directory, restart, and confirm creation precedes the `.gitignore` check. Verify the ignored case is quiet and the unignored case displays a warning while remaining active.
2. Put nested, hidden, and read-only files plus a directory link in the scratchpad. Restart, run `/new`, and shut down in separate runs; confirm every child is removed, the root remains, and link targets outside the root remain unchanged. Interrupt the process before graceful shutdown and confirm the next startup clears the residue before model use.
3. Configure an existing external directory from user configuration and confirm it activates. Configure a missing external directory and confirm Threadsmith does not create it, disables scratchpad for the session, and displays the reason. Attempt an external path from repository and session configuration and confirm rejection. Exercise filesystem/repository roots, `.git`, `.threadsmith`, prohibited paths, linked ancestors, and root replacement; confirm no external creation, traversal, or deletion occurs. Confirm `./.threadsmith/.scratchpad` activates and clears only its own contents, while `.threadsmith` and other descendants such as `.threadsmith/secrets` remain rejected.
4. At each repository trust level, use built-in `write_file` for eligible text beneath the active scratchpad and confirm no approval prompt. Confirm overwrite, extension, size, reserved-name, atomic-write, cancellation, and link checks still apply. Verify built-in `read_file` and `search`, including managed search fallback over an ignored in-repository scratchpad. Confirm sibling-prefix escapes, `list_files`, mutation, Git, process-specific grants, MCP, extensions, and same-ID lookalikes receive no scratchpad authority or listing disclosure.
5. Disable, deny, omit from the allowlist, or require approval for `write_file`; start a new session and confirm scratchpad remains configured but is absent from model context and inaccessible through all three tools. Disable `write_file` during an active session and confirm immediate revocation without mid-session reactivation; run `/new` after re-enabling and confirm a fresh capability.
6. Open a second repository with different scratchpad and tool settings. Confirm cleanup and warnings apply to the destination, tool invocations use destination configuration, and a forced late repository-open failure restores the original repository capability. Check `/new` and repository-open warnings in TUIKit and headless output. Inspect context provenance and confirm `System-Scratchpad.md` appears exactly once only while active, including child requests only when all three exact built-ins are assigned.
