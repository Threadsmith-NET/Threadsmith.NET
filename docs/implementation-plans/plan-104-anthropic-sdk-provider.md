...
**Status:** Complete; implementation, independent adversarial reviews, authenticated Sonnet verification, and the final solution regression run passed. The user-requested follow-up removes custom user secret-file permission rejection; its verification is recorded in §18.
...
**Execution boundary:** The original research publication changed planning Markdown only. Implementation was subsequently authorized on the local `plan-104-anthropic-sdk-provider` branch, stacked at `5f3a2aa` on the memory work. Authenticated live verification followed the user's configured key and Sonnet selection.
...
|---|---|
| New `src/Threadsmith.Models.Anthropic/` project, registration, request mapper/preparer, stream reader, replay codec, catalog client/cache, compatibility metadata, transport bridge | All Anthropic wire/SDK/configuration ownership; split cohesive classes rather than one large adapter |
...
9. **Close validation and documentation.** Run focused suites, architecture and relevant regression tests, release restore/license/package checks, and opt-in live checks when separately configured. Update shipped docs, acceptance/manual owners, and ADR as required. Record limitations and evidence in this plan; do not mark live verification complete based on fixtures.
...
Observable behavior is specified in Scenario AT. Operator procedures are MTP-263 (discovery and credentials), MTP-264 (thinking, tools, and privacy), and MTP-265 (usage, caching, and capacity). Live execution requires the user's configured key; fixture results do not stand in for a live API result.

- [x] An explicitly configured API key yields bounded API-based model discovery, stable model identities and honest eligibility through existing selection surfaces.
- [x] Selected Anthropic profiles use the official pinned SDK and direct Messages endpoint; existing providers and defaults behave as before.
- [x] Normal, planning/mutation, correction, skill, requested-child and auxiliary-summary paths use validated prepared requests and the correct trusted/ordinary dispatcher.
- [x] Text streams; complete tool calls execute only through host governance; multiple results retain exact correlation and valid chronology.
- [x] Users can choose whether future Anthropic responses include thinking summaries through `/thinking` or headless on/off. Requests send `summarized` when on and `omitted` when off, with no resubmission of in-flight requests. Supported reasoning levels remain selectable independently; signatures/private payloads never appear in the UI or durable state.
- [x] Tool continuations preserve required signed data exactly regardless of visibility, with bounded transient lifecycle and no durable/private-data leakage.
- [x] Compaction, selection changes, cancellation and restart never reuse mismatched signed state or repeat tool effects to reconstruct it.
- [x] Prompt caching uses valid explicit boundaries; enabling/disabling it preserves request meaning and capacity. Input/cache/output totals and conservative costs are correct.
- [x] Cancellation, deadline, classified retries, truncation, malformed streams and API failures produce bounded safe outcomes without duplicated output/tools.
- [x] SDK types/packages remain isolated; SDK disposal and ambient configuration do not affect the shared host transport or credential authority.
- [x] Focused and affected regression tests actually execute; opt-in live outcomes are separately recorded; source and packaged docs, ADR and release dependency evidence match implemented behavior.

Existing cross-cutting acceptance owners include Scenario R (model selection) and Scenario U (cache/context behavior). Scenario AT and MTP-263–265 own the native Anthropic behavior and procedures. Scenario T's Codex-specific contract and existing MTP identifiers are preserved.
...
The implementation boundary is recorded in [ADR-60](../architecture/adr-60-native-anthropic-provider-and-transient-replay.md), preserving ADR-29/30/41. Follow the current root `AGENTS.md` and shared C# guardrails; subtree instruction files were removed by a separate repository governance change. No new Anthropic system prompt is required merely to use the API. The existing mutation-proposal assets and both prompt catalogs now describe the native final-JSON path when that phase offers no proposal tool.
...
The original research publication added this plan and the minimal navigation/milestone entries. Implementation subsequently updated the user/operator docs, Scenario AT, and MTP-263–265. Completed milestone details remain unchanged.
...
The pinned package/source, dated exact-model capability/pricing policy, native strict-schema projection, conservative capacity interpretation, and transient replay contracts are implemented and covered by deterministic fixtures. If live upstream behavior conflicts with a specified contract, report the concrete conflict before changing scope or persisting private replay data.

## 18. Verification evidence

The implementation was built and tested directly in `C:\source\repos\Threadsmith` on `plan-104-anthropic-sdk-provider`. Independent protocol and host/catalog adversarial reviews returned clean after the relevant corrections. The initial full solution build completed with zero warnings and errors. The initial solution regression run executed 2,714 tests: 2,695 passed, zero failed, and 19 skipped (opt-in or platform-specific cases). This includes 147 native-provider tests, 12 native SDK/HTTP parent-loop integration tests, child-loop continuation coverage, and six catalog command tests. All 15 release contracts passed, and dependency evidence covers 52 projects.

Authenticated live checks on 2026-09-10 used the discovered exact model `claude-sonnet-5` and profile `397124d7-8633-51d9-84c8-bdea1bfd2980`. Discovery returned six eligible and five excluded models, and the selection survived catalog hydration. Existing default model selections were retained.

| Live check | Observed result |
|---|---|
| Required final JSON | Successful streamed response with the requested color and integer count |
| Two independent synthetic read-only tool calls | Both calls completed; correlated results produced the correct final color and count |
| Thinking visibility and continuation | Low-effort request with display omitted continued successfully after enabling display; a high-effort request returned 137 summary characters and two calls, then continued successfully with display omitted and zero summary characters |
| Stable-prefix cache | First request wrote 12,982 tokens; the identical repeat read all 12,982 tokens from cache. Reported costs were $0.032583 and $0.0027244 respectively |
| Cancellation and pooled transport | Cancellation interrupted one submitted attempt after 545 ms; a subsequent request through the same pooled client succeeded |
| Full application read-only request | Sonnet accepted all 26 tool schemas, completed `list_files` and two `read_file` calls against fresh synthetic text fixtures, and returned the correct color and count |

The first full request exposed an API validator incompatibility with a nullable scalar enum represented by an array-valued `type`. The native adapter now represents that enum using scalar-typed `anyOf` alternatives while preserving its exact values and nullability; canonical host schemas and validation remain unchanged. Seven regression cases cover this representation and preservation behavior. The focused native suite passed all 147 tests, the rebuilt solution had zero warnings/errors, and both final independent adversarial reviews returned clean.

Initial live checks used the supported process-environment secret provider because the former user-file permission policy rejected an additional Windows ACL principal. At the user's direction, that custom policy was removed: user-store access is now governed by the OS without ownership, ACL, or Unix mode inspection or changes. Direct file-source verification then succeeded with environment fallback disabled (`user-file:found`): Sonnet returned the expected JSON response using the existing key, and the file's permissions were unchanged. All 27 secret-resolution tests and 147 native-provider tests passed after this follow-up; architecture checks passed 218 tests with one opt-in skip. The Windows tests cover shared-ACL acceptance and an actual OS read denial, while the Unix mode case is conditional for Unix hosts. The full solution built without warnings or errors, and the independent follow-up review returned clean after removing an unused import. The successful full-application check used the active checkout's built application and fresh synthetic fixtures; product source was not copied into another checkout.

User-authorized follow-up live regression exercised the two existing providers through the full host against disposable synthetic repositories. Codex (`gpt-5.6-terra`) passed two-file reading, a same-session no-tool follow-up, one approved literal mutation and immediate undo with exact byte restoration, one read-only child-agent continuation, cancellation, and a successful next request. The thinking preference was toggled around cancellation; that check establishes request continuity, not native Anthropic thinking replay.

The OpenAI-compatible provider at the configured Ollama Node 2 endpoint passed two-file reading and same-session recall with `kimi-k2.7-code:cloud`. Kimi also emitted invalid plan shapes and paths, and repeatedly ignored the semantic-tool-first constraint during a child-agent request. Those attempts failed within host validation, and comparison with the prior committed code found the relevant schemas and correction rules unchanged. An explicit relative-path retry reached mutation preparation and exposed a pre-existing JSON property-order assumption: a valid operation discriminator appearing after ordinary properties was rejected. The mutation envelope parser now accepts that ordering while retaining strict member and mutation validation.

Using the same OpenAI-compatible transport, `glm-5.2:cloud` passed a read-only child-agent continuation and a thinking-enabled text request. No reasoning events were returned for that request, so visible reasoning was not established for that endpoint. Cancellation was accepted after approximately three seconds, and the next request in the same host returned the exact requested marker with no tools. These outcomes are recorded separately from model schema-adherence failures rather than treating provider transport success as a guarantee that every model follows the host contract.

After the parser correction, GLM also completed the approved `ShellRunner.Run()` literal change from `ready` to `verified`, followed immediately by the request to undo that change. The host recorded the execution outcome, both proposals stayed within the one-file fixture allowance, and the undo restored the exact original bytes. Missing operation discriminators were separately confirmed to raise `NotSupportedException`; both the native-tool and final-JSON envelope boundaries now translate that exception into the existing bounded schema-correction flow. Five focused regressions passed for late, missing, and unknown discriminators, the solution rebuilt with zero warnings/errors, and independent review of the final fix returned clean.

The follow-up full solution run executed 2,721 tests: 2,700 passed, two failed, and 19 were skipped. The failures were `RegisterExpectedWritesAsync_AttributesWatcherEchoToHostMutation` (watcher attribution) and one `ConversationalShell_AutomationTrust_RequiresExplicitChoice` row (timeout). The watcher case then passed alone, and all five trust-choice rows passed in isolation. Independent inspection found the watcher implementation/test and trust flow/test unchanged from the prior commit, consistent with timing sensitivity under suite contention. The original broad run remains recorded as failed; successful isolated reruns are separate evidence, not a retroactive clean full-suite result.

PR #31 review follow-up on 2026-09-10 corrected three valid findings: skill requests now use the selected profile's validated default reasoning level, the deployed continuation guidance is appended chronologically after native tool results, and headless `/models` returns an empty JSON array when active model selection is not composed. Provider status and refresh remain reachable for configured providers without selectable models. Eleven regression cases cover supported reasoning defaults across tool rounds, successful/denied tool continuation, offline/configured listing, maintenance, and invalid syntax. The new Anthropic cache/composition fixtures also use the test output directory to avoid macOS's symlinked temporary root while preserving the cache's link checks; macOS confirmation remains a CI obligation.

The review-fix solution build completed with zero warnings and errors. The full Windows regression run used `--max-parallel-test-modules 1` and executed 2,732 tests: 2,712 passed, 19 skipped, and one newly added skill fixture failed because it declared provider reasoning metadata using legacy markers. After correcting that fixture's schema marker, the entire Skills suite was rebuilt and all 56 tests passed. Every other test module passed in the broad run; the broad-run failure and successful Skills rerun remain distinct evidence. No live credentials or model calls were needed for these fixes. Existing CI failures, including the spike job referencing projects absent from `main`, are separate from the three resolved review findings.
