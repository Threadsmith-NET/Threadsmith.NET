...
Implementation adversarial review disposition: body limits now stop ADF traversal and bound repeated-mark expansion; blockquote prefixes stream within the UTF-8 body bound with periodic cancellation checks; table ordinals preserve leading empty rows and cells; media containers do not duplicate their child media placeholder; query/fragment decorations are ignored after route parsing; metadata-only fitting preserves body completeness; known ADF containers and empty labels fail or fall back honestly; merged-cell annotations stay within their cell; and an explicit tool-definition capability plus a per-tool child network allowlist admits Jira, but does not grant its network-host authority to other read-only tools. Deterministic regressions exercise sanitizer expansion, bounded blockquote formatting, table shape, media projection, ordinary conversation display, native-skill execution, child policy plus execution, cross-tool network denial, and body-free activity metadata. Live tenant and physical frontend checks remain governed by section 10.
