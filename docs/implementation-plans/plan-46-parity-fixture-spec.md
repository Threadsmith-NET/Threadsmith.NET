...
| `cloud-kimi-k2-7-code` | `kimi-k2.7-code:cloud` | `reasoning=true`; no model map; provider permits standard effort | `standard-effort`, selectable `None/Minimal/Low/Medium/High`: omit for `None`; otherwise emit the direct lowercase level | `reasoning-content` |
