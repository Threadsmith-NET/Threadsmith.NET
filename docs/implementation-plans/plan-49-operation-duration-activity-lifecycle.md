...
- Correct activity transitions across model → tool/MCP/active-turn compaction → continuation model → final response.
...
- `IConsoleSurface.ShowStatusUntilAsync` owns one static Spectre status while holding the serialized console gate. `ConversationalShell` owns activity replacement and transcript ordering, while `ConversationTranscript` currently materializes completed `THINKING` and generic tool markers. Plan 80 extends the same projection with content-free active-turn started/completed events: `COMPACTING CONTEXT` replaces `THINKING` only during candidate work, completion renders actual before/after/savings/profile/status plus authoritative candidate duration, and `THINKING` resumes. The later rendering tasks will replace the static string with one terminal-neutral dynamic activity model while preserving the existing single gate and stop/observe/write/start order; no timer tick becomes an event or transcript line.
...
- Active-turn candidate work temporarily displays `COMPACTING CONTEXT` with bounded before/target/profile/elapsed metadata, emits one content-free actual before/after/savings/status/profile/duration completion line, then resumes the original total-turn `THINKING` activity.
