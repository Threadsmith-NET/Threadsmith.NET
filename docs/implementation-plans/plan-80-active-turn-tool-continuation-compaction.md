...
- Candidate output selects request-local host-issued fact ids and allowed categories from its own result-specific hash record; the host—not the model—materializes exact content and attributable sources, carries prior validated items unchanged, and reports deterministic oldest-prior pruning.
