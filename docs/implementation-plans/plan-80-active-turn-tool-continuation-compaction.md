...
- Candidate output contains only a bounded array of request-local host-issued fact ids; the host—not the model—discards unknown/repeated/already-retained selections, drops excess valid ids beyond exact item/token bounds, assigns allowed categories and cumulative metadata, materializes exact content and attributable sources, carries prior validated items unchanged, and reports deterministic oldest-prior pruning.
