...
### Approved history follow-up

Checkpoint commit `f082bae` includes all 91 changed files preceding this follow-up. Ordinary children now use the shared active-turn summary generator for older completed exchanges, with independently configurable early triggers, recent-context targets, periodic attempts, measured savings before activation, and unchanged-history fallback. Initial instructions, task, supplied evidence, and steering stay exact. `read_agent_evidence` recovers results already delivered to the child, and provider history generations advance on replacement. This does not introduce a response format or semantic acceptance gate. The history implementation and subsequent skill-permission fix are included in rollback checkpoint `6e09349`.

The child loop now consumes the existing configurable output, tool-name, tool-argument, batch-count, and correction-length settings, including zero and the operational-limit off switch. The remaining shared tool/provider consumers below are separate work; this follow-up does not claim those complete.

Fresh verification for this follow-up: the solution build completed with zero warnings/errors; 1,027 regression tests passed (ParallelAgents 206, ModelTooling 401, CodexProvider 25, ConversationContext 81, Architecture 183, ContextCaching 33, Planning 98). Four unsupported filesystem cases and the separate opt-in live suite were skipped. Packaged documentation validation passed. Independent review feedback was resolved and the final source received a clean re-review. Child notes use sanitized tool content for code locations, omit a redundant file inventory, and retain every original evidence ID separately.

Approved live evaluation used four real-repository questions covering all six roles: the original readOnly/inherit audit, sibling cancellation, focused model selection, and skill trust/invocation. The original audit was also run with compaction disabled. Live failures led to fixes for lost usage on Codex output rejection, a duplicate delegation timer that ignored disabled child duration limits, unlimited fallback-summary projection, and a candidate-input cap that prevented later compaction after one large old exchange. Child summary input now defaults to the selected model's actual capacity (`maximumInputTokens=0`); the parent default is unchanged. An actual-generator integration case covers a 300,000-character complete exchange.

The final unchanged skills question completed with both reviewers: 50 requests, 1,517,584 input tokens, eight summaries/seven replacements, 62,080 peak child input, and no missing usage. Recorded initial instructions, call/result pairing, and request prefixes between replacements stayed intact. Earlier interrupted skills runs are retained as failure evidence, not successful benchmarks. A repeated audit had an architecture child stop after a bad argument before any compaction; the parent recovered answer coverage, but that requested independent review remained incomplete. The complete initial audit's peak child input was 44,078 versus 87,364 with compaction off; its known uncached input was higher, so reduced history size is not a proven billing or latency win. Final skills summaries alone used 271,638 input and 13,260 output tokens, included in the totals.

Independent utility review confirmed the principal cancellation/model-selection conclusions and the separate existing skill tool-allowlist replacement defect, while qualifying examples, citations, and the maintained-digest concern. The latter is an unused/misleading contract and conditional deployment concern, not a demonstrated authorization exploit. These skill findings were not silently added to this implementation's scope. Full questions, logs, measurements, failures, and analysis are retained under `C:\Users\mwrig\Documents\Codex\2026-09-05\loo\live-subagent-history`. The temporary project entry point was required because full-solution semantic startup failed before dispatch; the original solution setting was restored afterward. This completes the history follow-up's implementation/review/live evaluation, not the remaining plan-wide items below.

### Tool-argument follow-up

The two observed batch failures came from empty optional root paths. `list_files` and `search` now normalize omitted, null, empty, and whitespace-only paths to their existing root defaults before validation, scheduling, and execution. Nonblank paths, required arguments, and access restrictions are unchanged. Eight focused regression cases passed, alongside 798 passing affected regressions, two expected skips, a clean solution build, packaged documentation checks, and independent review. Tool descriptions and the user guide document the behavior; no forced retries or answer requirements were added. This fix remains uncommitted after `6e09349`.

Approved live reruns completed the original three-role audit and a focused TestReviewer question, with no invalid-argument corrections. The audit used 66 requests and 1,783,962 input tokens; the focused review used 40 requests and 1,182,132. Peak ordinary child inputs were 46,870 and 48,180. Every request reported usage, initial instructions and call/result pairing stayed intact, and all six evidence lookups exactly matched the originals. Neither run emitted an empty-string root path, so the focused regression cases provide the direct replay evidence. Independent utility review found useful role-specific work but qualified architecture claims, citations, and overstated test recommendations; no new security defect or required test expansion was established. Full measurements and qualifications are in `C:\Users\mwrig\Documents\Codex\2026-09-05\loo\live-subagent-history\argument-fix-evaluation.md`.

Efficiency remains unresolved: the focused review took 7.5 minutes, used six summaries, and repeated nine exact tool calls after earlier matching exchanges had left active history. Reduced request sizes and correct evidence retrieval do not prove lower cost or better reasoning. This does not establish that compaction caused the answer inaccuracies. The original solution selection was restored after testing; no persistent role or compaction settings changed.
...
- Finish and verify the approved disableable operational-limit changes in remaining consumers, including the advertised delegation input schema, result projection and rendering, shared tool budgets and file-tool limits, and remaining provider retry/transport settings. Option-binding tests alone do not verify those consumers.
