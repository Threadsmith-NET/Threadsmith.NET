...
Composition binds `execution:maxCorrectiveTurns` once in `HostFoundation.CreateAsync`. `ApplicationComposition` passes `host.ExecutionLimits.MaxCorrectiveTurns` into `ExecutionStartRequest.CorrectionBudget`; durable approved-plan validation cycles retain explicit checkpoint accounting while planning and mutation request corrections use `CorrectiveTurnState`. No legacy plan- or mutation-specific execution-limit properties remain.
...
- Do not bind obsolete per-loop keys or retain per-loop execution-limit properties; bind `execution:maxCorrectiveTurns` once and keep durable approved-plan checkpoint cycle accounting explicit.
