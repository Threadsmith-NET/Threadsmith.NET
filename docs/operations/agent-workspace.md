...
Automatic Git status refresh remains active at every trust level. Threadsmith's read-only Git queries force `core.fsmonitor=false` for each invocation so repository-configured fsmonitor hooks cannot execute during inspection. This override does not modify your Git configuration or affect status updates outside Threadsmith.
...
Run `/help` and confirm the command/description columns scroll inside the modal without adding transcript text. Open `/models` and verify provider/model labels and sorting. Exercise `/theme` as a single selection; toggle several hooks, extensions, MCP connections, and tools without closing their checkbox dialogs. Verify filtered group toggles and that Esc preserves completed changes. For an authorized OAuth profile, use F3 → Sign in / Authenticate, test cancellation back to the list, and confirm static-token profiles do not offer that action.

Request several read-only subagents. Confirm named queued/running rows appear within the live delegation block before all children finish, keep the original tool timer, and update once per agent. Verify final rows follow the completed timer, no GUID announcement appears, and tab gaps keep their bar style during selection and overflow. Exercise simultaneous tools, a failure/cancellation, and narrow-pane omission counts. With controlled usage reports, compare missing reasoning counts, reported zero, positive counts, and mixed known/unknown requests; output totals must not increase by the reasoning subset.
