...
17. If work remains, one new `propose_plan` call starts the ordinary sanity, approval, step, batch, mutation, and validation path again. If the objective is complete, an explicit no-argument `complete_objective` call causes the host to record terminal success. The final diff remains the net result relative to objective-start bytes rather than a concatenation of batch patches.
