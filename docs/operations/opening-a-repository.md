...
Previously persisted `TrustedBuild` or higher trust is reused on reopen from the per-user repository-facts database. Persisted `TrustedRead` opens an upgrade prompt where **Keep Trusted Read** retains text-only discovery, **Upgrade to Build** enables compiler-aware discovery, and **Cancel** leaves the repository closed. Trust is monotonic: requesting a lower level does not erase a persisted higher grant. The configured `solution:path` remains the first candidate. Discovery recursively finds solution containers below the repository while skipping `.git`, `bin`, `obj`, reparse points, and unreadable/reserved-name entries. A single candidate is selected automatically; multiple candidates require explicit selection. During initial TUIKit startup, successful repository/solution loading and remembered-solution details remain only in the splash. Required choices precede the splash, and typing/paste while it is active is discarded. Warnings and failure diagnostics remain in MAIN; explicit repository changes after startup report their results normally. See [workspace startup behavior](agent-workspace.md).
