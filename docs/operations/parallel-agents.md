...
Ask Threadsmith for parallel work in a trusted repository with a selected semantic workspace. When useful, the parent model can invoke the built-in `delegate_agents` tool with one to five children by default. Independent assignments belong in the same `agents` array to run concurrently, subject to configured concurrency. Separate calls in one model-response batch run sequentially. The parent can request several assignments in one call:
...
For tab names, per-role name lists, suffix allocation, focus keys, per-agent usage, and display limits, see [agent workspace operations](agent-workspace.md). Child request counters include provider-reported reasoning only when available; these tokens are already part of output usage.
...
Trusted machine/user configuration controls ordinary conversation delegation under `agents:delegation`. Existing defaults allow five children, 4,096 task characters, 8,192 context characters, 1,024 summary characters, and a five-minute deadline. Positive values enforce their caps; zero disables that configurable cap. When a count or text cap is disabled, the advertised `delegate_agents` schema omits the matching `maxItems` or `maxLength` keyword instead of publishing a stale compiled limit. An unrestricted final body does not disable active scheduler, request, tool, transport, or provider controls. Mutation, process, build, and test tools can be inherited when the parent's advertised tools and permissions allow them; explicit `readOnly` excludes execution and writes. Every eligible parent evidence item and every resolved `AGENTS.md` and configured prompt append source is included in the child request. Repository configuration cannot replace these trusted settings, and none are model-authored fields.
...
Ordinary child conversations can summarize inherited evidence and older completed tool exchanges after delivery, while retaining their task, instructions, recent work, and steering. Inherited results are delivered in full before they become eligible, and their original evidence IDs remain retrievable. The global trusted `context:activeTurnCompaction:profileId` and optional `reasoningLevel` select the same compaction model for the main loop and all child roles. With no global profile, each child uses its task model and reasoning. Role model assignments do not override an explicit global compaction model. Configure child history tuning under trusted `agents:delegation:compaction` independently from operational limits; see [Subagent history and evidence](../user-guide.md#subagent-history-and-evidence) for defaults, off switches, and exact-result lookup. Working notes and the evidence-ID index are in-memory request state, not a resumable checkpoint or a substitute for stored evidence. Raw model diagnostics show summary requests as `active-turn-compaction-policy` messages and child replacements through `HistoryRewriteGeneration`. Include those auxiliary requests when comparing input, cached input, output, and elapsed time; input already includes cache reads, so do not add cache reads again.

Failed attempts may still consume tokens. Count any usage chunks emitted before failure, without counting the same usage again from a response summary. If a stream ends before usage arrives, mark that attempt unknown and the totals as lower bounds. Compare answer quality and cache reuse as well as request size: summary overhead and cache misses can outweigh the saved input.
