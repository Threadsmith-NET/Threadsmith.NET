...
1. Ensure the repository is clean, the intended commit is on `main`, and all normal CI jobs pass. Review `eng/release/ripgrep-assets.json`: it must name only official `BurntSushi/ripgrep` release assets, retain the approved `MIT OR Unlicense` contract with MIT selected for distribution, and pin an independently reviewed SHA-256 digest for every supported RID. Separately review `src/Threadsmith.Reranking.Local/crossencoder-assets.json`: it pins the official Apache-2.0 cross-encoder revision, model and vocabulary sizes, and SHA-256 digests.
