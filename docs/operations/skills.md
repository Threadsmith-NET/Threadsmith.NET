...
Enable `delegate_agents` and the tools you want the model to use. The skill inherits the session's enabled tool surface under its normal policy; it can choose Git tools or `run_process` for acquisition. Remote fetch guidance requests only branch tips and no history. Tools and child agents use their ordinary lifecycle, logs and UI. Children use configured role models or the frozen root model/reasoning as fallback.

The model reads code and configuration, assesses possible credential markers without exposing values, and synthesizes the child responses into Markdown. If `.inbox` exists it can save with `write_file` using explicit content; otherwise it returns the report to the window. A failed save is explained with the report shown in the window. All specialists failing produces a failed/red skill result rather than a successful review.
