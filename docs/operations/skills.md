...
Invoke the maintained `review` package explicitly with `/skills use Maintained:review <JSON>` or the model's existing `invoke_skill` tool. The three input modes are:

```json
{"mode":"currentBranchChanges","baseBranch":"main"}
{"mode":"remoteBranch","repository":"https://github.com/team/project.git","branch":"feature/search"}
{"mode":"specialInstructions","instructions":"Review cancellation ownership","paths":["src/Search"],"requirementsDocumentPath":"docs/requirements.md"}
```

Omitting `mode` selects current branch changes. That mode captures the branch's committed delta from its merge base together with eligible staged, unstaged and untracked content. `baseBranch` is explicit, or resolved from a single locally available remote default-branch symbolic reference. Missing/ambiguous metadata and detached HEAD without an explicit base fail before launch. No `main`/`master` guess is made.

Remote mode requires an exact repository URL (HTTPS or SSH) or configured remote name, plus an exact branch. It fetches into a separate host-managed bare repository without altering the invoking checkout, index, refs or remote configuration. No base means an audit of the branch snapshot, including existing defects; `baseBranch` requests a merge-base change review. Credentials come from existing Git configuration; URLs containing secrets are rejected. Read-oriented acquisition does not execute hooks, project code, submodules or LFS downloads. Network/executable grants remain authoritative. An unavailable ref/history or interrupted fetch fails rather than selecting a fallback branch.

Special instructions review the current snapshot by default; adding a base requests a change review. `paths` narrows the captured source and finding scope while applicable ancestor instructions and the selected target requirements remain available. Unknown paths fail input validation. Text cannot change tool authority, role/model routing or report formatting.

The four existing security, test, performance and architecture roles each receive one verified private procedure, the same immutable source and explicit review facts. They receive no parent conversation, author summary or sibling procedure. Each uses the existing role/model/reasoning policy and the same child loop. Their only source reader serves captured text. Individual reads that would be changed by redaction are rejected before citation authorization. Source that secret redaction would alter is excluded with a coverage limitation instead of being presented as modified code; a requirements document that needs redaction is rejected for replacement with a sanitized source. Governing instructions or Git path/ref identities that cannot be captured unchanged prevent reviewer launch. They cannot mutate source, fetch websites, run tests/builds/benchmarks, invoke skills or delegate again. Native JSON Schema and delivered-source-range checks reject malformed or fabricated output. Trusted `skills:review:maximumFormatCorrections` optionally permits that many same-conversation corrections; its default is zero. Corrections and tool usage remain metered.

Four roles are accounted for even when child-count/concurrency limits require batches. One failure does not discard valid sibling findings. Reports distinguish partial coverage and exclusions from complete static inspection; neither means approval or proof of correctness.

`requirementsDocumentPath` accepts bounded Markdown/plain text under current read grants. Relative paths use the invoking workspace, including remote reviews. Set `requirementsSource` to `reviewTarget` to use the captured branch's document. The source digest and explicit acceptance criteria are frozen before review. All extracted criteria appear in the report with `Met`, `Partially met`, `Not met`, `Not assessed`, or `Ambiguous`, source lines, evidence and gaps. Duplicate IDs, unsupported formats and missing/unreadable documents fail honestly. No document means no acceptance section; a document without explicit criteria produces a visible limitation. Static review cannot attest runtime or manual verification.

Reports always contain `What's good`, `Overall architectural soundness`, `Possible issues` with P1/P2/P3 groups, and `Observations`, in that order. Positive/architectural claims require inspected evidence. Nondefect, speculative and intent-dependent suggestions remain observations. An optional acceptance table follows. Host rendering escapes source-controlled Markdown and identifies captured revisions.

If the invoking repository already contains an authorized `.inbox` directory, Threadsmith atomically saves a unique UTF-8 report there and returns a concise clickable receipt. Remote targets still use that invoking local inbox. If the directory is absent, or no local invoking repository exists, the canonical report goes to console output. Threadsmith never creates `.inbox`. A conflicting file, link or unwritable directory is a delivery error, with the canonical report retained for repair/retry; it is not silent console fallback.

Version-2 private review records reject obsolete capture semantics on resume and live under the host's local application-data `Threadsmith/reviews` directory. Explicit resume revalidates package pins, snapshot identity and current authority. Started model loops are not automatically restarted after interruption; durable valid children can be joined with honest missing coverage. Delivery retries reuse the saved content identity and destination without inference or overwriting an existing different report.

The public catalog gains only `review`. Private packages are deployed separately under `ReviewSkills` and cannot be selected through public skills commands. Existing `review-pr`, other native packages, Claude-compatible packages and ordinary `delegate_agents` role responses are unchanged. This is an optional local review workflow; CI/SARIF, PR publication and merge gates remain separate. See [ADR-61](../architecture/adr-61-focused-review-skill-assignments.md).

Headless invocation uses the same `/skills use Maintained:review <JSON>` command with the existing repository/trust flags. It does not require selecting or building a solution. The declared skill model-turn, tool-call and finding budgets are divided across the four reviewers; ordinary telemetry-only child budgets stay unchanged.

The explicit review entry has host defaults of 64 total model turns and 128 tool calls, capped by the public recipe and each private procedure; callers that supply an explicit budget keep it. A selected path scope captures its source, any selected target requirements document, and applicable ancestor `AGENTS.md` guidance. It does not read unrelated source merely to filter findings afterward.
