...
| `code_explore` | `TrustedBuild` | None | Exact C# anchors with bounded current source, optional compiler-proven multi-anchor flow, dispatch branches, and compact impact evidence. |
...
`code_explore` captures one semantic workspace generation and emits current source only for exact C# symbol, stable-ID, or `.cs` path anchors. With multiple resolved anchors, `Flow` mode composes bounded compiler-proven call paths, typed call sites, interface/virtual dispatch branches, and explicit runtime/static-boundary omissions; `Impact` mode adds compact callers, implementations, direct/transitive dependent projects, and tests as planning evidence. It does not perform natural-language discovery, restore/build/generator execution, mutation, process, network, runtime tracing, or implicit text fallback; stale, prohibited, oversized, or policy-denied source/flow/branch/impact evidence is omitted with explicit reasons.
