...
| `code_explore` | `TrustedBuild` | None | Exact C# symbol, stable-ID, or repository-relative path anchors with bounded current line-numbered source, digests, confidence, omissions, and continuations. |
...
`code_explore` captures one semantic workspace generation and emits current source only for exact C# symbol, stable-ID, or `.cs` path anchors. It does not perform natural-language discovery, restore/build/generator execution, mutation, process, network, or implicit text fallback; stale, prohibited, oversized, or policy-denied source is omitted with explicit reasons.
