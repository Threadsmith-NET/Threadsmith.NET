...
Checked-in examples live in `tests/fixtures/scripts/` and cover success, missing usage, transient provider failure, malformed output, and cancellation. Fixtures must contain placeholders rather than secrets. `Threadsmith.Milestone1.Tests` copies and executes these files as part of the build gate.
