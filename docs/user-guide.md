...
| `/context map` | Explain the latest actual MAIN request's context usage with ordered source sizes and aggregate category shares. |
...
Double-click MAIN's `Context` meter to open `/context map`, including while execution continues. The large native-chart modal freezes the latest prepared request, preserves your draft, and shows estimates rather than a next-request preview. Individual contributions form a flat top-to-bottom stack in model-context order: provider instructions, host policy, repository instructions, canonical native tools, phase policy, conversation history, included memories, governed state, current user input, and continuations; empty entries are omitted. The initial host-stable block is marked with `│` and ends with `└`, with a reconciled token total labeled `cache eligible`. This indicates likely reuse, while actual cache use requires provider telemetry. Larger category groups and totals remain in the right-hand summary. Mouse capture must be enabled (F12). Use arrows/PageUp/PageDown to navigate, Tab for the complete category ranking, and Esc to close. See [context usage and accounting](operations/cache-optimized-context.md#context-inspect).
...
`/tools`, `/hooks` (also `/hooks list`), `/mcp` (also `/mcp list`), MCP tool availability, and `/extensions` use keyboard-only checkbox trees. Arrows navigate; Space applies each toggle immediately and leaves the modal open for more changes. Typing/paste filters, Backspace edits the filter, F2 shows details, and Esc/Enter closes while keeping completed changes. Group toggles affect only visible eligible members: Space disables them if any are enabled, otherwise it enables them. Batch progress shows completed and rejected counts; Esc stops after the current item finishes and keeps the tree open. Checked means tool available, hook enabled, MCP profile connected, or extension loaded. Hook enablement does not grant repository approval; connecting a server does not enable its tools or change startup auto-connect. Failed operations restore the reported host state. `/theme` is a single-selection modal.
...
Submitted conversation input, including slash commands, is echoed into MAIN with exactly one blank line before it. This spacing is preserved after terminal resizing.
...
All seven roles below are available to ordinary conversation delegation. The role names are exact and case-sensitive; `explorer` is the default when `role` is omitted. Unknown roles, different capitalization, and unknown request fields are rejected.
...
| `bugReviewer` | Compare implementation against supplied requirements, acceptance criteria, and expected behavior. Find concrete functional bugs and regressions, citing code and requirements such as Jira ticket details passed by the lead. | Cannot invent missing requirements, change files, or claim unperformed validation. |
...
These restrictions apply to every ordinary role, not just reviewers. A role does not unlock extra tools, and `inherit` does not grant the parent's write or command-execution permissions. A child cannot choose a different model, enable tools, expand its access, or change its assignment. Model choices for all seven roles come from the configuration described below, not from the child's reply.
...
`dotnet_inventory` projects the authoritative selected loaded semantic workspace into deterministic solution/project, TFM, project-reference, package-reference, central-version-source, and test-project results. It returns every loaded project and every direct package reference, plus all centrally pinned package IDs in the root `Directory.Packages.props`, including unused pins. Caller-supplied path text cannot replace selected-solution provenance. It reports semantic confidence and omissions; degraded or absent workspace state is not presented as complete evaluation. Every selected solution, loaded project, and `Directory.Packages.props` metadata access is confined by approved-root, prohibited-path, and reparse-point policy before bounded reads; inventory never restores packages or queries latest available versions. An output that exceeds the tool transport bound fails explicitly rather than silently dropping packages.
...
`diagnostic_query` pages the bounded process-local exploratory index by invocation/run, project, file, code, severity, compiler/analyzer origin, and baseline class. Its optional model-facing `runId` is a UUID string and retains historical filtering. `test_discover` enumerates one confined supported test project without restore/build, derives host-issued repository-bound identities, and can narrow results by exact namespace, class, method, or available trait metadata. `test_run_targeted` accepts the returned identity as a direct string, rechecks its project against current path policy, and generates an exact effective filter. Unknown, cross-repository, expired, or ambiguous identities fail closed.
...
The unique matching account is selected from trusted URL patterns; supply `provider` when configured patterns are ambiguous. Supported PR subviews and fragments normalize to the canonical PR URL. A single call returns the complete requested evidence after internal provider pagination and consistency checks. Use `refresh: true` to discard and reacquire the operation snapshot.

Live progress identifies the account, inventory/diff kind, acquisition status, refresh state, and canonical URL. Successful results contain complete acquired evidence, source/destination commit identities, and provider limitations. Parent and skill executions reuse the operation cache under ordinary tool and network policy; children receive the acquired evidence through delegation. See [pull-request retrieval](operations/pr-fetch.md) for URL routing, cache lifetime, bounds, and provider-specific authentication.
...
|---|---|
| `query` | Required non-empty plain-text string, at most the configured character limit (default 500, provider maximum 600) and 75 whitespace-delimited words. |
...
Threadsmith translates locale hints into Brave's supported search-language codes; for example, `en-US` sends language `en` and country `US`. A region that Brave does not support as a country hint leaves the language hint in effect. Chinese region hints select simplified (`zh-CN`) or traditional (`zh-TW`/`zh-HK`) search, while bare `pt`, `zh`, and `no` map to `pt-pt`, `zh-hans`, and `nb`. Freshness windows of 1, 7, 31, and 365 days use Brave's presets; other windows use an invariant UTC date range. The host defaults to 500 query characters; `webSearch:provider:maximumQueryCharacters` can raise this to the provider's 600-character maximum. The provider's 75-word limit also applies. See the [Brave Web Search API contract](https://api-dashboard.search.brave.com/api-reference/web/search/get).
...
After login, Threadsmith reads the latest stable Codex release version from OpenAI's GitHub repository, then queries the protected Codex `/models` resource with that compatibility version and projects every distinct model returned for the account. If release lookup is unavailable, it uses a compiled known-working version. There is no hard-coded Codex model list. Stable Threadsmith profile IDs are derived deterministically from provider/model identity; returned context and reasoning metadata is bounded and normalized. Credential-free model metadata is cached under the user-owned `~/.threadsmith` directory and becomes selectable at the next process start. Logout removes both Threadsmith's Codex grant and this metadata cache. Repository provider catalogs cannot replace the host-owned `openai-codex` definition.
...
Hex colors require true-color terminal output. TUIKit detects modern Windows consoles even when debugger/default-terminal launches omit terminal environment markers; Linux and macOS use terminal capability signals such as `COLORTERM=truecolor`. Startup and theme changes visibly report color approximation on limited terminals and deliberate color suppression.

Each role accepts optional `foreground` and `background` colors plus the Boolean decorations `bold`, `dim`, `italic`, `underline`, `strikethrough`, and `invert`. An omitted or `null` color inherits. Role styles inherit only from the selected theme's `Default` role; there is no fallback to `system` or any other theme. Colors omitted by both use terminal defaults; decorations omitted by both are off. Foreground and background inherit independently. Decorations inherit as a set: omit all decoration flags to inherit `Default`, or specify any flag to replace the inherited set with the flags explicitly set to `true`. A single `bold: false`, for example, clears inherited decorations. Built-in themes define their Markdown decorations explicitly; custom themes must specify them if desired. Built-in themes do not enable inversion for any role; opt in with `invert: true` in your theme. Configuring colors alone on `SessionStatus`, `TitleBarRole`, or `AgentSelectedTabRole` no longer inherits reverse video.
