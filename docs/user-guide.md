...
`code_explore` requires an opened `TrustedBuild` semantic workspace and accepts exact C# symbol anchors, stable symbol IDs from prior semantic results, or repository-relative `.cs` path anchors with optional line/range selection. It returns one generation-fenced result containing resolved anchors, stable identities, project/TFM, generated/linked classification, line-numbered current source, file and range SHA-256 digests, completeness dimensions, omissions, and replayable path continuations. Its `Auto`, `Survey`, `Flow`, and `Impact` modes can additionally compose bounded compiler-proven paths among resolved anchors, call-site/source linkage, interface/virtual implementation branches, explicit delegate/dynamic/runtime boundaries, and compact callers, implementations, direct/transitive dependent projects, and tests. This evidence is planning context, not whole-program proof or mutation/test authorization. Unanchored natural-language discovery is intentionally unsupported in this foundation and returns guidance rather than pretending prose is an exact anchor; use semantic discovery or text search first when only a bare filename or concept is known. Before source or identity-bearing flow/impact evidence is emitted, current confined file content and path policy must still allow it; changed, prohibited, oversized, or policy-denied evidence is omitted with explicit reasons.
