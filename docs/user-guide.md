...
`code_explore` requires an opened `TrustedBuild` semantic workspace and accepts exact C# symbol anchors, stable symbol IDs from prior semantic results, or repository-relative `.cs` path anchors with optional line/range selection. It returns one generation-fenced result containing resolved anchors, stable identities, project/TFM, generated/linked classification, line-numbered current source, file and range SHA-256 digests, completeness dimensions, omissions, and replayable path continuations. Unanchored natural-language discovery is intentionally unsupported in this foundation and returns guidance rather than pretending prose is an exact anchor; use semantic discovery or text search first when only a bare filename or concept is known. Before source is emitted, the current confined file content must still match the captured semantic text; changed, prohibited, oversized, or policy-denied source is omitted with explicit reasons.
