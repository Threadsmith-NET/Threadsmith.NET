...
2. **Plan proposal** — the model calls the host-owned `propose_plan` tool with the summary, steps, file intents, expected outcomes, validation, risks and questions. The host assigns schema version, revision and step IDs. Text that merely describes edits is not enough to enter mutation flow.
...
4. **Implementation proposal** — after manual or policy plan approval, the host refreshes only the approved file endpoints from disk, so a previous edit or manual rollback is reflected in the new execution baseline. The TUI shows `MUTATION: Generating edits` while the parent model receives eligible source evidence and proposal-only `propose_mutations`. The model supplies changes; the host correlates them to accepted plan steps. The host binds hashes, byte lengths, existing-file identities, lifecycle classification, validation policy, and replacement lengths from the captured baseline and existing policy. For a unique nonempty `expectedText`, `startOffset` may be omitted. The host computes the UTF-16 range, avoiding model character counting. Empty insertions and repeated anchors require an exact offset. New-file content defaults to UTF-8 without BOM and LF when formatting is omitted; a move without content preserves bytes, while move-with-content preserves source encoding/BOM and the submitted line endings unless explicit formatting is requested.
