...
7. **Resolve and validate every selected fact.** The host derives closed factual fragments from each exact sanitized result, including its content SHA-256 and bounded JSON leaves or text lines, and assigns each projected fragment a short request-local fact id. The candidate returns only a fact id and one category allowed for that fact; it does not copy nested fact text or source objects. The host resolves each id to byte-exact content plus one deterministic attributable evidence/invocation source before applying the unchanged strict validator. Unknown ids, fabricated selections, modified headers, unsupported categories, unsafe content, authority-bearing claims, and source-range/version mismatches are rejected.
