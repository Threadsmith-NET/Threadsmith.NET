...
The tool accepts several anchor styles:
...
- **Exact symbol anchors** — simple names, qualified names, overload-like signatures, or stable symbol IDs returned by earlier semantic tools.
- **Repository-relative `.cs` path anchors** — optionally with a line, exact line range, tail window, containing declaration, or whole-file selection.
- **Associated artifact path anchors** — explicit prompt/config/project artifact paths to include alongside the selected C# slice.

It returns one generation-fenced result. The result includes query interpretation, resolved anchors, stable semantic identities, project/TFM, generated/linked classification, selected and retained candidate summaries, allocation summaries, line-numbered current source or precise current-context back-references, file and range SHA-256 digests, completeness dimensions, omissions, emitted-range records, and replayable continuations.

Modes adjust the emphasis:

- **`Auto`** — default source-bearing survey for the resolved query.
- **`Survey`** — prioritizes declaration/source context around anchors.
- **`Flow`** — composes bounded compiler-proven call paths between multiple resolved anchors, including call sites, direct/static calls, interface/virtual dispatch branches, and explicit delegate/dynamic/runtime boundaries where static proof stops.
- **`Impact`** — returns compact blast-radius evidence: callers, implementations, direct/transitive dependent projects, and tests with reasons. This is planning evidence, not whole-program proof or mutation authorization.
...
`code_explore` can also return **associated non-C# artifacts** as a separate supplement to the C# semantic spine. Supported associations include explicit artifact paths, Roslyn additional documents, analyzer configuration documents, loaded project metadata files, bounded textual project item/resource references, source-literal repository-relative paths, logical prompt/configuration names, and bounded exact-name matches under selected source/project directories. Returned artifacts are labeled with relationship kind, evidence strength, origin C# symbol/file/range, media kind, current digest, line range, completeness, and omission reasons; raw project-file item text is marked as weaker textual inference when item conditions/imports/removes have not been evaluated. Prompt templates, JSON/YAML configuration, XML/resx/project metadata, Markdown, schemas, and text templates remain untrusted repository data: Threadsmith never executes, evaluates, imports, renders, expands external entities, or grants authority from their contents.

Source and artifact output have independent request limits and may also be clamped by the selected model budget. Overflow is reported through omissions and continuation targets. Artifacts are omitted when they are binary-shaped, malformed, oversized, missing, changed during read, outside approved roots, prohibited, reparse/device paths, secret/credential-shaped, Git metadata, build output, generated transient output, unsupported media, or unavailable through bounded inventory. Exact-name artifact lookup uses declared, policy-allowlisted host-owned Git inventory when available, parses NUL-framed records before sanitization, caches repeated directory inventories within the query, and fails closed if inventory cannot be trusted.
