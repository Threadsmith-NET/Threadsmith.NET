...
2. **Complete plan proposal** — the model calls the host-owned `propose_plan` tool with the complete summary, ordered steps, file intents, expected outcomes, validation, risks and questions. Every accepted or corrected candidate represents the whole plan. The host assigns schema version, revision and step IDs. Later steps are not generated lazily during implementation, and text that merely describes edits is not enough to enter mutation flow.
3. **Whole-plan sanity and approval policy** — the host validates plan schema 2, runs cheap repository sanity checks over every structured `fileIntent` (`Modify`, `Create`, `Delete`, `Move`, or `Rename`), classifies risk, and either returns repairable scope failures to the model for a bounded complete-plan revision, prompts for review, or auto-approves according to `/plan-policy` / `planning:approvalPolicy`. Approval covers the whole plan and authorizes implementation work only, not repository writes.
...
13. **Correction or terminal completion** — introduced compiler/test failures can stage a bounded correction against the active step and promoted transactional baseline while retaining the original diagnostic basis and batch completion intent. Every correction repeats proposal validation, screening, diff, policy, transaction, and validation. The run succeeds only after every approved step has supported completion; the final diff is the net current result relative to execution start rather than concatenated historical batch patches.
...
- `execution:mutationBatching` supplies positive soft targets for operations, distinct paths, and mutation-content characters in each active-step proposal. These settings tune mutation batches only; they neither split the up-front plan nor replace hard workspace limits.
