...
| **Cumulative summary** | One bounded, explicitly untrusted historical assistant message that replaces an eligible prefix. A later compaction carries its validated items forward and adds facts from the next eligible prefix. |
...
6. **Generate a structured candidate.** An actual candidate attempt uses the separately configured compaction profile when present, or the active main profile as a backward-compatible fallback. The candidate profile may identify a different model or provider and owns candidate context/reserve, maximum output, reasoning, temperature, timeout, retry, sensitivity, and pricing. Candidate requests advertise no tools, use the `Summary` workload when independently configured, and cross the normal managed before/after model-request hook boundary. They are real model requests: profile identity, reported usage, missing usage, call count, duration, and any partial usage emitted before a later failure are accounted under each attempt's unique identity. One transient or malformed-JSON retry is allowed, for at most two candidate calls.
7. **Resolve and validate every selected fact.** The host derives closed factual fragments from each exact sanitized result, including its content SHA-256 and bounded JSON leaves or text lines, and assigns each projected fragment a short request-local fact id. The candidate returns only a bounded `factIds` array; it does not supply categories, cumulative headers, nested fact text, or source objects. The host discards unknown, repeated, and already-retained selections; greedily retains valid ids in model order only while exact item-count and rendered-summary token bounds fit; assigns deterministic allowed categories and cumulative metadata; resolves each retained id to byte-exact content plus one deterministic attributable evidence/invocation source; and applies the unchanged strict validator. A response with no usable ids, unsafe content, authority-bearing claims, or a host-derived source/range/version mismatch is rejected.
...
| Per-result factual support | Up to 64 exact facts of at most 2,000 characters each |
| Candidate attempts | One initial call plus at most one transient or malformed-JSON retry; two calls total |
...
| `InsufficientSavings` | A valid candidate failed the pressure-target or 4,096-token savings gate. |
