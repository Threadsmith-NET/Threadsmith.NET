...
`code_explore` requires an opened `TrustedBuild` semantic workspace and accepts natural-language C# architecture/behavior queries, exact C# symbol anchors, stable symbol IDs from prior semantic results, or repository-relative `.cs` path anchors with optional line/range selection. It returns one generation-fenced result containing query interpretation, resolved anchors, stable identities, project/TFM, generated/linked classification, selected and retained candidate summaries, allocation summaries, line-numbered current source or precise current-context back-references, file and range SHA-256 digests, completeness dimensions, omissions, deduplication counts, reclaimed-budget-use counts, emitted-range records, and replayable path continuations. Its `Auto`, `Survey`, `Flow`, and `Impact` modes can additionally compose bounded compiler-proven paths among resolved anchors, call-site/source linkage, interface/virtual implementation branches, explicit delegate/dynamic/runtime boundaries, and compact callers, implementations, direct/transitive dependent projects, and tests. Natural-language terms drive deterministic Roslyn-backed declaration discovery and ranking when exact anchors are not known; query text is inert data and is never executed, provider-reranked, or treated as an unbounded text search. Ranking uses compiler-known declarations and allowed paths, with exact/pinned evidence, qualified names, distinctive identifiers, multi-term/co-located structure, graph connectivity, and explicit generated/test focus outranking isolated common-word collisions. Source and metadata are bounded by request limits and selected-model budget; overflow is reported through omissions and continuation targets. On overlapping follow-ups, unchanged complete ranges are suppressed only when the host proves the exact source is still present verbatim in the current canonical model request for the same repository, workspace, generation, path, and digest, and only when the serialized pointer is smaller than the replaced source. Back-reference range digests describe the exact advertised range, including covered subsets of larger prior ranges. Edits, compaction, different sessions/repositories, short spans, partial or structurally reduced prior output, pointer-larger-than-source cases, and uncertainty cause safe re-emission or omission; `UsedForNewSourceCharacters` counts only later source that fit because suppression freed capacity. This evidence is planning context, not whole-program proof or mutation/test authorization. Before source or identity-bearing flow/impact evidence is emitted or pointerized, current confined file content and path policy must still allow it; changed, prohibited, oversized, or policy-denied evidence is omitted with explicit reasons.
