...
- `release/` — release target matrix; reviewed license evidence, deterministic SPDX/notices, exact runtime legal staging, artifact compliance; self-contained publish/staging; complete prompt-catalog and packaged-documentation validation; Windows, Linux, and macOS packaging; signing/notarization hooks; aggregate provenance; and release contract/smoke checks.
...
- Every staged or extracted payload must contain the manifest-controlled `ThreadsmithDocs` bundle. Validation rejects missing required files, unmanifested or excluded content, unsafe links/reparse points, broken or escaping local Markdown links, and payloads above the bounded file/byte limits.
