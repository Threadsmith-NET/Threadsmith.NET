...
foreach ($required in @($appHost, $workerHost, $ripgrepHost, (Join-Path $stage 'config.example'), (Join-Path $stage 'providers.example.json'), (Join-Path $stage 'LICENSE'), (Join-Path $stage 'ThreadsmithDocs/manifest.json'), (Join-Path $stage 'third-party/ripgrep/LICENSE-MIT'), (Join-Path $stage 'third-party/ripgrep/UNLICENSE'), (Join-Path $stage 'third-party/ripgrep/SOURCE.json'), (Join-Path $stage 'third-party/THIRD-PARTY-NOTICES.txt'), (Join-Path $stage 'third-party/sbom.spdx.json'), (Join-Path $stage 'third-party/dotnet-runtime/LICENSE.txt'), (Join-Path $stage 'third-party/dotnet-runtime/THIRD-PARTY-NOTICES.txt'), (Join-Path $stage 'release-compliance.json'))) {
