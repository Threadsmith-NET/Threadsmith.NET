...
Search iteratively before answering. Search from `.`, which already includes descendant documentation; do not repeat the same query under `docs`. Derive distinct high-signal variants from the question, including literal command syntax when applicable and noun, verb, or established product-term forms. For example, a question about compacting context should search both `/context compact` and `compaction`. A match that merely repeats the question, describes this help skill, or points to a historical plan is not answer evidence. Use search result line numbers to read the smallest relevant Markdown section, normally no more than 120 lines, when surrounding context is needed. Base citations only on search or read_file results. Supply the closest governing Markdown heading when it is visible; the host derives the authoritative heading from the cited range. Keep snippets concise and relevant. Exact source text is preferred, and `...` may omit intervening text. Before returning unavailable, try at least two distinct relevant search terms unless the documentation bundle itself cannot be read.
...
Return a concise answer with status answered, partial, or unavailable. For answered or partial results, cite one to eight bundle-relative paths, Markdown headings, one-based line ranges, and short relevant snippets. The host treats the path and range as authoritative and canonicalizes headings; snippet wording is presentation text. When any cited evidence is relevant but incomplete, use partial and state the gaps. Use unavailable only with an empty citations array when no shipped evidence answers the question or the documentation bundle cannot be read. Never attach citations to unavailable, never guess, and never cite implementation plans.
'@
        'schemas/input.json' = @'
{
  "type": "object",
    ...
    "citations": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "heading": { "type": "string", "minLength": 1, "maxLength": 256, "description": "Closest governing Markdown heading when known, including its # markers. The host canonicalizes this value from the cited range." },
          "lineStart": { "type": "integer", "minimum": 1, "maximum": 1000000 },
          "lineEnd": { "type": "integer", "minimum": 1, "maximum": 1000000 },
          "snippet": { "type": "string", "minLength": 1, "maxLength": 500, "description": "Concise relevant citation text. Exact source text is preferred; ... may elide intervening text." }
        }
      }
    },
