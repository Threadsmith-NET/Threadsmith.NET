        ...
        CancellationToken cancellationToken = default)
    {
        ...
        switch (command.Action)
        {
            case ModelProviderAuthenticationAction.Status:
                OpenAiCodexAuthenticationStatus status = await oauth.GetStatusAsync(cancellationToken);
            ...
            case ModelProviderAuthenticationAction.Login:
                ...
                Uri reservation = listener.ReserveRedirectUri(1455);
                var redirectUri = new Uri($"http://localhost:{reservation.Port}/auth/callback");
                OpenAiCodexAuthorizationChallenge challenge = OpenAiCodexOAuthManager.CreateBrowserChallenge(redirectUri);
                await new SystemBrowserLauncher().LaunchAsync(challenge.AuthorizationUri, cancellationToken);
                Uri callback = await listener.WaitForCallbackAsync(reservation, cancellationToken);
                ...
                OpenAiCodexProviderConfiguration catalog = await new OpenAiCodexCatalogClient(httpClient)
