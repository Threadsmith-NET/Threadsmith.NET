...
Own phase-specific evidence governance, bounded cross-turn conversation memory, repository-scoped memory policy, model-context assembly, prompt assets, and per-request model resolution.

## Ownership

- EvidenceStore.cs — attributable session-scoped evidence, clone copying, and turn-boundary invalidation.
- ContextAssembler.cs — phase policy, three-mode conversation assembly, repository-memory retrieval, deterministic pressure reduction, token estimation, telemetry, and inspection records.
...
- Stable host policy precedes the untrusted content-addressed repository instruction bundle, which precedes phase instructions. It requires an advertised semantic tool whenever one covers the repository question; text search is allowed only when none applies or the attempted semantic tool fails or explicitly reports incomplete/degraded evidence, and equivalent searches must not repeat after sufficient semantic evidence. Evidence-collection guidance tells the model Threadsmith has fast host-native inspection tools and should batch independent read-only inspections in one response, prefer structural/semantic/index tools where applicable, and avoid serial one-search/one-file or adjacent narrow reads. Once target, applicable instructions, and material impact resolve a requested repository change without a correctness ambiguity, stop tool exploration and call `propose_plan` rather than investigate unrelated patterns or references. Read-only exploration, audits, explanations, and diagnostics should answer directly from gathered evidence instead of manufacturing a plan. The execution host advertises inspection tools for the full model-round budget by default; context guidance must preserve the direct-answer path for read-only work, while remaining compatible with the optional positive `execution:maxPlanningToolRounds` cutoff. Resolve applicable `AGENTS.md` parent-to-child from the canonical root to working scope; configured prompt appends follow with distinct provenance.
