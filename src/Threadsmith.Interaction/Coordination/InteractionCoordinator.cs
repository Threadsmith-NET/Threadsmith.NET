...
public sealed class InteractionCoordinator
{
    ...
    /// <param name="surface">Frontend-neutral interaction surface.</param>
    /// <param name="modelCatalog">Configured model catalog, or <see langword="null" />.</param>
    /// <param name="activeProfileId">Active model profile identity, or <see langword="null" />.</param>
    /// <param name="sessionPreferences">Mutable session model preferences, or <see langword="null" />.</param>
    /// <param name="extensionManager">Extension manager, or <see langword="null" />.</param>
    ...
    /// <param name="displayOptions">Immutable shared display behavior.</param>
    /// <param name="displayWarnings">Bounded frontend configuration warnings.</param>
    /// <param name="timeProvider">Clock used for monotonic activity timing.</param>
    /// <param name="gitQueries">Local Git query boundary.</param>
    ...
    public InteractionCoordinator(
        ...
        InteractionDisplayOptions? displayOptions = null,
        IReadOnlyList<string>? displayWarnings = null,
        ...
        CodeExploreOutputOptions? codeExploreOutputOptions = null)
    {
        ...
        _displayOptions = displayOptions ?? new InteractionDisplayOptions();
        _displayWarnings = displayWarnings ?? [];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        InteractionRepositoryOpenWorkflowResult? activeRepository = null;
            ...
            async (batch, token) =>
            {
                ...
                foreach (var domainEvent in batch.Where(item => item.SessionId == sessionId))
                {
                    ...
                    if (domainEvent is SemanticCheckStarted semanticStarted)
                    {
                        ...
                        incomingActivity = new InteractionActivity(
                        ...
                        incomingActivity = domainEvent switch
                        {
                            ...
                            ModelReasoningObserved when !streamThinking && turnStartedTimestamp is { } turnStart =>
                                new InteractionActivity(
                                    ...
                                    _displayOptions.ShowOperationDurations,
                                    _timeProvider),
                            ActiveTurnCompactionStarted started => new InteractionActivity(
                            ...
                            MutationProposalStarted => new InteractionActivity(
                                ...
                                _displayOptions.ShowOperationDurations,
                                _timeProvider),
                            _ when directFetchApprovalGranted => new InteractionActivity(
                                "TOOLS: web_fetch",
                                _timeProvider.GetTimestamp(),
                                _displayOptions.ShowOperationDurations,
                                _timeProvider),
                    ...
                    {
                        output.Add(new PresentationTextItem(
                        [
                    ...
                    if (streamingThinkingActive && domainEvent is not ModelReasoningObserved)
                    {
                        if (!streamingThinkingEndedWithLineBreak)
                        {
                            output.Add(new PresentationTextItem(
                            [
                                new PresentationTextSegment(Environment.NewLine, PresentationTextRole.Reasoning),
                            ]));
                        }
                    ...
                    else
                    {
                        ...
                        if (domainEvent is ModelReasoningObserved reasoning && streamThinking)
                        {
                            ...
                            {
                                eventSegments.Add(new PresentationTextSegment(
                                    Environment.NewLine,
                                    PresentationTextRole.Reasoning));
                                streamingThinkingActive = true;
                            }

                            streamingThinkingEndedWithLineBreak = text.EndsWith('\n');
                            eventSegments.Add(new PresentationTextSegment(text, PresentationTextRole.Reasoning));
                        }
                        else
                        {
                    ...
                    {
                        nextActivity = new InteractionActivity(
                            "THINKING",
                            continuationStart,
                            _displayOptions.ShowOperationDurations,
                            _timeProvider);
        ...
        try
        {
            while (!lifetime.IsCancellationRequested)
            {
                ...
                try
                {
                    if (_showSessionStatus)
                    {
                        ...
                        var repositoryName = activeRepository?.Repository is null
                            ...
                            ? "Not open"
                            : SessionStatusAssembler.GetRepositoryDisplayName(activePath);
                ...
                var thinkingCommand = input.Kind == InteractionInputKind.ToggleThinking
                    || (commandText.StartsWith("/thinking", StringComparison.OrdinalIgnoreCase)
                        && (commandText.Length == 9 || char.IsWhiteSpace(commandText[9])));
                if (thinkingCommand)
                {
                    var argument = input.Kind == InteractionInputKind.ToggleThinking || commandText.Length == 9
                        ? string.Empty
                        : commandText[9..].Trim();
                ...
                if (InteractiveCommandRouter.TryParse(commandText, out var frontendInvocation)
                    && frontendInvocation is { Descriptor.IsFrontendLocal: true })
                {
                    if (_frontendCommands is null
                        || await _frontendCommands.HandleAsync(
                            frontendInvocation,
                            _surface.Surface,
                            lifetime.Token) != FrontendCommandOutcome.Handled)
                    {
                        await _surface.WriteAsync(
                    ...
                    && (commandText.Length == 5 || char.IsWhiteSpace(commandText[5])))
                {
                    ...
                    {
                        var pathInput = await InteractionInputReader.ReadSecondaryAsync(
                            _surface,
                            "Repository path:\n",
                            PresentationTextRole.Status,
                            ...
                            if (steeringWaitTask?.IsCompleted == true)
                            {
                                    ...
                                    && pendingSteeringPauseId is { } readyPauseId)
                                {
                                    ...
                                    try
                                    {
                                        while (true)
                                        {
                                            var steeringInput = await InteractionInputReader.ReadSecondaryAsync(
                                                _surface,
                                                null,
                                                PresentationTextRole.Status,
                                            ...
                                            if (steeringInput.Kind == InteractionInputKind.ToggleThinking)
                                            {
                                                continue;
                                            }
...
#pragma warning restore VSTHRD003
        ...
        IReadOnlyList<PresentationItem> output)
    {
        ArgumentNullException.ThrowIfNull(surface);
        ArgumentNullException.ThrowIfNull(output);
        PresentationItem[] safeOutput =
        [
            .. output.Select(item => item is PresentationMarkdownItem markdownOutput
                ? new PresentationSourceItem(
    ...
    private static bool TryGetLatestSemanticActivity(
        IReadOnlyDictionary<SemanticActivityKey, InteractionActivity> activitiesByKey,
        IReadOnlyList<SemanticActivityKey> activityOrder,
        out SemanticActivityKey key,
        out InteractionActivity? activity)
    {
    ...
    private static void RemoveSemanticActivitiesForRun(
        IDictionary<SemanticActivityKey, InteractionActivity> activitiesByKey,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await _surface.WriteAsync(
            ...
            result.Succeeded ? PresentationTextRole.Status : PresentationTextRole.Error,
            cancellationToken);
    }
            ...
            switch (action)
            {
                ...
                case "disable":
                    ...
                    await _surface.WriteAsync(
                        ...
                        changed ? PresentationTextRole.Status : PresentationTextRole.Error,
                        cancellationToken);
                    break;
                case "test":
                ...
                case "approve":
                    ...
                    await _surface.WriteAsync(
                        ...
                        approved ? PresentationTextRole.Status : PresentationTextRole.Error,
                        cancellationToken);
                    break;
                case "revoke":
            ...
            switch (operation.ToLowerInvariant())
            {
                ...
                case "inspect":
                    ...
                    await _surface.WriteAsync(
                        ...
                        inspected is null ? PresentationTextRole.Warning : PresentationTextRole.Status,
                        cancellationToken);
                    return;
                ...
                case "forget":
                    ...
                    await _surface.WriteAsync(
                        ...
                        forgotten ? PresentationTextRole.Status : PresentationTextRole.Warning,
                        cancellationToken);
                    return;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (argument.StartsWith("mode ", StringComparison.OrdinalIgnoreCase))
        {
            ...
            await _surface.WriteAsync(
                ...
                changed ? PresentationTextRole.Status : PresentationTextRole.Warning,
                cancellationToken);
            return;
        }
        ...
        try
        {
            switch (operation.ToLowerInvariant())
            {
                case "list":
                    ...
                    await _surface.WriteAsync(listing, PresentationTextRole.Status, cancellationToken);
                    return;
                ...
                case "uninstall":
                    ...
                    await _surface.WriteAsync(
                        ...
                        removed ? PresentationTextRole.Status : PresentationTextRole.Warning,
                        cancellationToken);
                    return;
                ...
                case "disable":
                    ...
                    await _surface.WriteAsync(
                        ...
                        changed.Enabled ? PresentationTextRole.Status : PresentationTextRole.Warning,
                        cancellationToken);
                    return;
                ...
                case "status":
                    ...
                    await _surface.WriteAsync(
                        ...
                        checkpoint is null ? PresentationTextRole.Warning : PresentationTextRole.Status,
                        cancellationToken);
                    return;
                ...
                case "cancel":
                    ...
                    await _surface.WriteAsync(
                        ...
                        cancelled ? PresentationTextRole.Status : PresentationTextRole.Warning,
                        cancellationToken);
                    return;
    ...
    private async Task HandleAgentsCommandAsync(
        InteractionController controller,
        DelegationActivityRegistry delegations,
        SessionId sessionId,
        string commandText,
        CancellationToken cancellationToken)
    {
        ...
        {
            await _surface.WriteAsync(
                delegations.FormatSummary(sessionId),
                PresentationTextRole.Status,
                cancellationToken);
            return;
        }

        if (!Guid.TryParse(arguments[0], out var delegationValue))
        {
            await _surface.WriteAsync(
    ...
    private async Task SetRepositoryPromptAsync(
        ...
        CancellationToken cancellationToken)
    {
        var repositoryName = SessionStatusAssembler.GetRepositoryDisplayName(repositoryPath);
    ...
    private async Task ManageExtensionsAsync(InteractionController controller, CancellationToken cancellationToken)
    {
    ...
    private async Task<InteractionRepositoryOpenWorkflowResult> OpenRepositoryAsync(
        InteractionController controller,
        string repositoryPath,
