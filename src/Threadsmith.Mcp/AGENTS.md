...
- OAuth cache mutation is exact-profile scoped. Logout persists prefix tombstones so credentials found through the compatibility secret-store fallback cannot be resolved again after local removal. Logout makes no remote claim; revocation uses request-timeout-bounded non-redirecting metadata and a same-origin HTTPS RFC 7009 endpoint. Network failures and host-owned timeouts are unconfirmed remote outcomes; explicit local-cleanup authorization must still clear the exact profile cache.
