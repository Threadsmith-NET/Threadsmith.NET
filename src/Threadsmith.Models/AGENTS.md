...
- `Prompts/` — provider-neutral textual tool-inventory fallback assets; filename and token authority remains in the Core catalog.
...
- Selection is limited to `ConfiguredModelCatalog`; user defaults and extension/skill hints cannot introduce endpoints, models, or credentials. Configured provider dispatch may resolve the host-owned active profile for each new request, but an explicit request-resolved profile remains authoritative for that in-flight boundary. When request constraints force a different profile than the active preference, Execution must commit that fallback through the shared active-selection boundary and reassemble before provider I/O; it may not silently dispatch while leaving the prior profile active.
...
- Optional request-owned provider instructions carry one stable section id and exact loaded content before admission. Wire estimation counts their tokens in total input, section, and stable-prefix accounting; summaries expose only safe size/token metadata, while explicitly enabled raw request logging retains the provider-visible body.
