...
- `PackagedDocumentationPolicy` applies only after catalog resolution proves both maintained scope and the normalized `threadsmith-docs-help` id. Its procedure search/read calls are rebound to application-owned `ThreadsmithDocs`; repository packages with a colliding id retain ordinary repository scope. Cited paths and ranges must resolve inside the confined bundle. The host canonicalizes headings when the source provides one, opportunistically expands recognizable snippet elisions, and normalizes status/citation consistency; presentation-level model choices are not invocation failure boundaries.
...
- `SkillBudget.ContentTokens` bounds loaded package assets only. Procedure prompts and tool continuations are admitted against the selected model's actual wire context window and output reserve; do not derive a second character ceiling from the package-content budget.
...
- Intersect declared tool IDs with the current invocation context's non-empty allowlist and deny rules before advertisement, and refresh that context again before execution. An empty computed intersection must set `DenyAllTools`; an empty original host allowlist retains its existing unrestricted-by-ID meaning. Preserve other authorization fields and apply the same rules to restored workflows and maintained documentation procedures.
