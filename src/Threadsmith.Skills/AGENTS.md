...
- `PackagedDocumentationPolicy` applies only after catalog resolution proves both maintained scope and the normalized `threadsmith-docs-help` id. Its procedure search/read calls are rebound to application-owned `ThreadsmithDocs`; repository packages with a colliding id retain ordinary repository scope. Cited ranges must stay within one governing Markdown section, and the host replaces model-supplied heading text with the exact governing source heading.
