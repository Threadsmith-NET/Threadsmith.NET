...
- Retained diagnostic runs and stable test identities are repository-bound; diagnostic queries filter by the active normalized repository before pagination, and test identities include repository identity before resolving their project through current invocation path policy.
